...
export function SettingsRoot() { const [model,setModel]=useState<SectionsModel>(); const [ideBrowserFocusNonce,setIdeBrowserFocusNonce]=useState<number>(); const strings=(window as any).__TACHYON_STRINGS__ as CockpitStrings; useEffect(()=>{ const onMessage=(e:MessageEvent)=>{if(e.data?.type===MODEL){setModel(e.data.model);setIdeBrowserFocusNonce(e.data.ideBrowserFocusNonce)}}; window.addEventListener("message",onMessage); post(readyMessage()); const timer=setInterval(()=>post(pollSettingsAction()),3000); return()=>{clearInterval(timer);window.removeEventListener("message",onMessage)}},[]); useEffect(()=>{ if(model?.globalSettings) applyTachyonFont(model.globalSettings.fontMono); },[model?.globalSettings?.fontMono]); useEffect(()=>{if(ideBrowserFocusNonce===undefined)return;const block=document.querySelector<HTMLElement>('[data-testid="control-settings-ide-browser"]');const toggle=block?.querySelector<HTMLInputElement>('[data-testid="ide-browser-enabled-toggle"]');if(!block)return;block.scrollIntoView({block:"center",behavior:"smooth"});block.classList.remove("ck-settings-highlight");void block.offsetWidth;block.classList.add("ck-settings-highlight");toggle?.focus({preventScroll:true});const timer=window.setTimeout(()=>block.classList.remove("ck-settings-highlight"),2400);return()=>window.clearTimeout(timer)},[ideBrowserFocusNonce]); return model?<main class="ds-page"><SettingsApp model={model} strings={strings} post={post}/></main>:null; }
