 ...
 * are TOML-escaped. A leading `${PLUGIN_ROOT}/…` in command/args is rewritten to the absolute materialized
 * payload root (t-b6180e) — the runtime does not receive the placeholder. http auth maps to codex's structured
 * fields (no free-form header string): an `Authorization: Bearer ${VAR}` → `bearer_token_env_var`; every other
 * header (incl. a bare-token Authorization) → an `env_http_headers` inline table (header name → env-var name).
 * Pure; the merge into the file (Step 3) replaces only this block.
 * @param pluginRoot absolute workspace payload root (`.tachyon/plugins/<name>`); required when the server uses `${PLUGIN_ROOT}`.
 */
export function renderCodexMcpBlock(server: McpServer, pluginRoot?: string): string {
  const resolved = withResolvedMcpPluginRoot(server, pluginRoot);
  const lines = [`[mcp_servers.${resolved.name}]`];
  if (resolved.transport === "stdio") {
    lines.push(`command = ${tomlStr(resolved.command)}`);
    if (resolved.args.length > 0) lines.push(`args = [${resolved.args.map(tomlStr).join(", ")}]`);
    ...
    lines.push(`url = ${tomlStr(resolved.url)}`);
