 ...
 *
 * Formation's lifecycle host reads this module (`createFormationLifecycleHost` →
 * `isNativeSuppressionConfirmed`). The suppression receipt covers every enabled human
 ...
 *
 * t-4c3d90 — this file is the production door. It lived only in `test/helpers/` after the monorepo
 * cut, and the host wired `verifyNativeSuppression: () => false`. Do not move it back to tests.
 */
