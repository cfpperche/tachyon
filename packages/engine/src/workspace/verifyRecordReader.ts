 ...
 *
 * Why the host can read a record an AGENT's worktree produced: refs live in the common git dir,
 * which is SHARED by every worktree of a repository. The gate runs in the agent's own checkout and
 * updates the ref there; the host, running in the primary checkout, reads the same ref. Without that
 * property this whole signal would be invisible to the host and the feature would need a second
 * channel.
 *
 ...
 * its own ref name, an unparseable/stale/future timestamp, an unattributable schema, or any git
...
/** Must match `REF_PREFIX` in `scripts/verify-record.mjs`, which owns the record layout. */
const RECORD_REF_PREFIX = "refs/tachyon/verify";
...
export async function readVerificationRecord(
  ...
  try {
    const raw = await gitLine(git, ["cat-file", "blob", `${RECORD_REF_PREFIX}/${tree}`], worktreePath);
    if (!raw) return undefined;
    const record = JSON.parse(raw) as
      Partial<VerificationRecord> & { schema?: number };
    // A record that does not name the tree its ref names proves nothing about that tree.
