...
export const strings: InspectorStrings = {
  ...
  empty: "No Tachyon sessions on the socket. Start an agent to populate the server.",
  ...
  groups: [
    {
      ...
      sessions: [
        ...
        { session: "tachyon-cmd-a1b2c3d4-test", kind: "unknown", label: "test", pid: 5001, dead: true, exitCode: 1, currentCommand: "npm test", startCommand: "npm test", createdAt: now - 30 },
      ],
    },
    {
...
const volumeGroups: InspectorModel["groups"] = [
  {
    ...
    sessions: [
      ...
      { session: "tachyon-cmd-a1b2c3d4-verify", kind: "unknown", label: "verify", pid: 5001, dead: false, currentCommand: "vitest", startCommand: "npm run verify:full:quiet", createdAt: now - 45, cpu: "busy" },
      { session: "tachyon-cmd-a1b2c3d4-typecheck", kind: "unknown", label: "typecheck", pid: 5002, dead: true, exitCode: 0, currentCommand: "", startCommand: "npm run typecheck", createdAt: now - 900 },
      { session: "tachyon-cmd-a1b2c3d4-test", kind: "unknown", label: "test", pid: 5003, dead: true, exitCode: 1, currentCommand: "npm test", startCommand: "npm test -- --reporter=verbose test/unit/webviewConvention.test.ts", createdAt: now - 30 },
      { session: "tachyon-rb-a1b2c3d4-release-1", kind: "unknown", label: "release · package", pid: 5101, dead: false, currentCommand: "vsce", startCommand: "npx @vscode/vsce package --no-dependencies --out dist/tachyon.vsix", createdAt: now - 210, cpu: "busy" },
      { session: "tachyon-rb-a1b2c3d4-release-2", kind: "unknown", label: "release · audit", pid: 5102, dead: true, exitCode: 0, currentCommand: "", startCommand: "node scripts/audit-vsix.mjs", createdAt: now - 260 },
  ...
  },
  {
    ...
    sessions: [
      ...
      { session: "tachyon-cmd-d4e5f6a7-lint", kind: "unknown", label: "lint", pid: 6002, dead: false, currentCommand: "eslint", startCommand: "npm run lint -- --max-warnings=0", createdAt: now - 20, cpu: "busy" },
      { session: "tachyon-cmd-d4e5f6a7-build", kind: "unknown", label: "build", pid: 6003, dead: true, exitCode: 2, currentCommand: "", startCommand: "npm run build:docs", createdAt: now - 7200 },
  ...
  },
  {
    ...
    sessions: [
      { session: "tachyon-ff009911-old-spike", kind: "session", label: "old-spike", pid: 3300, dead: false, currentCommand: "bash", startCommand: "bash", createdAt: now - 90000, cpu: "idle" },
      { session: "tachyon-cmd-ff009911-migrate", kind: "unknown", label: "migrate", pid: 3301, dead: false, currentCommand: "node", startCommand: "node scripts/migrate-storage.mjs --dry-run --verbose", createdAt: now - 90200, cpu: "idle" },
      { session: "tachyon-rb-ff009911-nightly-3", kind: "unknown", label: "nightly · sweep", pid: 3302, dead: true, exitCode: 137, currentCommand: "", startCommand: "node scripts/nightly-sweep.mjs", createdAt: now - 91000 },
