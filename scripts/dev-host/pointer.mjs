 ...
 *   (CLI: scripts/dev-host/cli.sh point|point-status|point-clear|fixture-new)
 *
...
export async function status(repoRoot, opts = {}) {
  ...
  if (!fs.existsSync(p.meta)) {
    return {
      armed: false,
      broken: true,
      reason: "no meta.json — run: scripts/dev-host/cli.sh point …",
      checkout: resolved,
      warnings: [],
    };
  }
...
`);
  ...
  if (sub === "fixture-new") {
    ...
    console.log(`  then: scripts/dev-host/cli.sh point --worktree … --fixture ${result.slug.replace(/-dogfood$/, "")}`);
    return 0;
  }
