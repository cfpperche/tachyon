...
export const ROOT_SHELL_ALLOW = Object.freeze([
  {
    prefix: "src/webview/",
    reason: "Compatibility contract for callers checking historical root-shell fixtures; the repository has no root src/.",
  },
]);

export function checkVscodeImportBoundaries(root = process.cwd(), shellAllow = ROOT_SHELL_ALLOW) {
  const liveShellAllow = fs.existsSync(path.join(root, "src")) ? shellAllow : [];
  const scopes = [
    // Kept as an empty compatibility scope so fixture callers can still prove a supplied shell
    // allowlist is live. The repository itself owns no root `src/` and supplies no entries.
  ...
  const rootFiles = filesByScope.flatMap((scope) => scope.files).map((file) => relative(root, file));
  const staleAllowEntries = liveShellAllow.filter((entry) => !rootFiles.some((file) => file.startsWith(entry.prefix)));
  const offenders = [];

  for (const scope of filesByScope) {
    for (const file of scope.files) {
      const rel = relative(root, file);
      if (scope.allowShell && liveShellAllow.some((entry) => rel.startsWith(entry.prefix))) continue;
...
function main() {
  ...
  if (result.staleAllowEntries.length) {
    process.stderr.write("engine-boundary: FAIL — shell allowlist entries match zero source files:\n");
  ...
  process.stdout.write("engine-boundary: OK — engine and webview-ui have zero vscode imports (value or type)\n");
}
