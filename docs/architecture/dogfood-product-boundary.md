...
| `verify_task` default stays generic (`npm test`), never Tachyon's build-aware `verify:full` | 🔒 locked | `DEFAULT_FULL_VERIFY` in `src/bridge/verifyTask.ts` pinned `=== "npm test"` by `test/unit/snBoundaryLocksBehavior.gen.test.ts`; Tachyon itself opts into `verify:full` via its own `tachyon.yml` `settings.verify`, not the default. |
