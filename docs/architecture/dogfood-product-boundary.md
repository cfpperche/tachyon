...
| The packaged VSIX STARTS, and its non-engine doors answer | 🔒 locked at release | `npm run smoke:vsix -- <path.vsix>` installs the artifact into a disposable editor (`--extensions-dir`/`--user-data-dir`), boots an extension host on a private X server, and opens one door per live surface via `test/vsix-smoke/probe/suite.js`: activation, every declared command registered, every contributed view focused. Measured warm on 0.57.1: 3.6s for 4 doors. Deliberately NOT in `verify:full` — `assertStableBuildSource` refuses to build a package from a linked worktree or off `main`, so at gate time there is no artifact to smoke; this is a release step, between `vsce package` and the human. Bound stated in the probe: the workspace carries no `tachyon.yml`, because a packaged (`stable`) build launches its engine daemon from a copy of `process.execPath`, which in a DESKTOP extension host is the Electron binary and cannot start — engine-backed doors are unreachable here, and a red that means "wrong host" is a red people learn to ignore (t-e0a0f5). |
