...
| Packaged VSIX matches what the build claims, and keeps every promise the engine manifest makes | 🔒 locked | `scripts/vsix-artifact.mjs` unpacks the release candidate and compares its real bytes with the embedded provenance record, with every `engine-manifest.json` entry, and with the no-source-map rule for the engine payload. `record-provenance.mjs audit` runs it **before** writing the audit record and refuses to write one on mismatch, so a green audit cannot describe an artifact that does not match it. `test/unit/vsixArtifactAudit.test.ts` pins seven refusal shapes (absent claim, changed bytes, unkept manifest promise, manifest hash mismatch, shipped source map, engine claims with no manifest, unreadable archive) plus the healthy case. Re-run against the real 0.56.102 artifact: it is refused for the exact promised-but-pruned `app.js.map` that crashed activation, while 0.56.103 and 0.56.105 pass (256 and 308 files checked). Replaces the manual unzip that this row previously depended on (t-1f425c). Its runtime-module check was rewritten by t-e0a0f5: comparing every bare `require` in `dist/extension.js` against the package refused BOTH real 0.57.x releases over `bufferutil` and `utf-8-validate` — optional native speedups inside `ws` that belong in no VSIX — while missing the `ws` defect that actually shipped, because that one was reached through a dynamic `import(...)`. `scripts/package-closure.mjs` now attributes each import to the module that wrote it, over every node bundle in the package. |
| The packaged VSIX contains everything its own code loads, and everything its manifest names | 🔒 locked | `scripts/package-closure.mjs` reads esbuild's per-module provenance comments, so a bare specifier can be attributed to the file that asked for it: ours (`src/…`) must resolve inside the package; a third-party library's optional load is that library's business. Re-run against the real artifacts — `tachyon-0.57.0.vsix` is refused for `ws <= src/webview/ide-browser-bridge/cdpSession.ts`, the exact defect that shipped, and `tachyon-0.57.1.vsix` passes with no first-party violation. `test/unit/packageClosure.test.ts` pins the 0.57.0 shape byte-for-byte (`Function('return import("ws")')()`), the t-09a462 external shape, an unattributable import (fail-closed), and the four third-party optionals that must NOT be flagged — a checker that cries about those gets silenced, and the next `ws` walks past it. Reached through two doors: `record-provenance.mjs audit` and `npm run smoke:vsix` (t-e0a0f5). |
| The packaged VSIX STARTS, its doors answer, and its ENGINE runs on an Electron host | 🔒 locked at release | `npm run smoke:vsix -- <path.vsix>` installs the artifact into a disposable editor (`--extensions-dir`/`--user-data-dir`), boots an extension host on a private X server, and opens one door per live surface via `test/vsix-smoke/probe/suite.js`. Deliberately NOT in `verify:full` — `assertStableBuildSource` refuses to build a package from a linked worktree or off `main`, so at gate time there is no artifact to smoke; this is a release step, between `vsce package` and the human. t-a8e1f7 removed the bound this row used to state. The workspace now carries a `tachyon.yml`, so the packaged `stable` build must stage a runtime and launch its daemon, and the run happens TWICE: with Node on PATH the daemon answers `commands.list` with the declared command, and with every Node-holding PATH entry pruned activation is refused in 0ms by `EngineBundleError` instead of timing out at 12.8s. Both runs pass `--force-disable-user-env`, because VS Code otherwise re-probes the login shell and hands the host a PATH the runner never set — measured, and the first refusal run was green-for-the-wrong-reason until it was found. Re-run against the real `tachyon-0.63.0.vsix`: 6 doors, PASS, 6.1s warm for both runs. That is the t-d11d57 fix proven on the host format where the engine used to die, and the engine is retired with its run (`systemctl --user stop`), so a release leaves no daemon behind. |
...
| A published plugin's manifest loads through Tachyon's own parser | 🔒 locked | `src/pluginValidateEntry.ts` calls the real `loadManifest` — never a second copy of the schema, which would drift and then report green while the loader refuses — and is bundled standalone to `dist/plugin-validate.cjs` (`npm run plugin:validate <dir…>`) because the plugins repository has no Node toolchain and Tachyon is not on npm. `test/unit/pluginValidateEntry.test.ts` pins five refusals: the exact `{kind, path}` shape that shipped as v1.0.0, a declared `leaf` absent from the package, an unknown hook event, invalid JSON, and a directory with no manifest — plus that the entry imports the shared loader. On the author's side, `scripts/validate-manifests.sh` + a `pre-push` hook in `cfpperche/tachyon-plugins` refuse a push to its trunk when any package would be refused, and refuse equally when the validator cannot be found ("could not check" is not "checked"). Verified against reality: all 17 published packages load, and re-injecting the v1.0.0 shape makes the hook exit 1 (t-d8e772). |
| Project guidance is explicit project input, not Tachyon product policy | 🔒 locked | `test/unit/projectGuidanceOwnership.test.ts` proves absence for an unconfigured consumer, synthetic exact ordered/source-labelled delivery, and that Tachyon's real `tachyon.yml` opts into its owned document with exact provenance-labelled composition. Was `PI-001` until 2026-07-25, when the maintainer retired the Product Invariant ceremony as too heavy for a beta project; the coverage moved to the ordinary suite unchanged, so this row is still held by a forcing function. |
...
| `contributes.configuration` defaults carry no Tachyon-repo assumptions | 🔒 locked | Satisfied absolutely since `t-aaad95`: there is **no `contributes.configuration`** left, so no contributed default can carry an assumption. `test/unit/snBoundaryLocksBehavior.gen.test.ts` asserts its absence; `test/unit/settingsAuthorityInventory.test.ts` is the inventory guard that keeps every reader, key and localization string from growing back. Tachyon's settings live in `tachyon.yml` (project) and `~/.tachyon/settings.json` (per person), neither of which is a VS Code contribution. |
...
- **verify-gate v1.0.0 uninstallable (2026-07-25):** the manifest declared `{kind, path}` — what the
  parser *produces* — where the input contract wanted `{leaf}`. The package could not install in any
  environment. The gate script itself had been exercised across all six push shapes before publishing;
  what went untested was the PACKAGE, because nothing in the publication path ever handed a manifest to
  `loadManifest`. Validating the input by running the artifact's *contents* is not the same as loading
  the artifact (t-d8e772).
