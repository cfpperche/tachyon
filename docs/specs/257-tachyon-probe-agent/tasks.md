...
- [x] Capability/compat gated by **automated binary-gated live smokes** (`test/unit/probeSmoke.test.ts` — real `claude`/`codex`/`grok --version` via the adapters, skipped when absent) + recorded versions (`codex-cli 0.142.0`) + golden-fixture mocks + an opt-in real end-to-end run (`PROBE_LIVE_SMOKE=1`, verified: codex freeform → ok). Not mocks alone. (D5)
- [x] **t-7426de** — Grok headless adapter + schema + Cap 13 parity (2026-07-24)
