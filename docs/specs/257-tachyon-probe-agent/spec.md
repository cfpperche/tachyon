...
**Status:** DRAFT — probe-reviewed (an adversarial cross-model probe reshaped the decisions below). Two maintainer confirmations remain, marked **[CONFIRM]**. · **Follows:** [spec 250](../250-tachyon-plugin-system/) (the per-runtime adapter pattern this reuses) and the Bridge A2A surface (`spawn_agent` / `wait_for_agent` / `read_output`) which this complements, not replaces. Relates to verify-gate ([spec 214](../214-tachyon-verify-gate/)) and pipelines (`src/pipeline/`), both future consumers of a clean captured result. · **Surface:** a shared internal run model (`AgentRun`), a thin `probe_agent` Bridge tool + a result reader, a per-runtime "headless capture" adapter capability, and a ledger-backed observability row. · **UI impact:** flow (a transient probe row rendered FROM the run ledger; a way to inspect a finished probe's captured result).
...
- **D10 — Scope: two runtimes, duet-first. [CONFIRM]** MVP ships **both** claude + codex adapters (the cross-runtime duet is the headline value — a single-runtime MVP proves capture but not the thesis); it **defers** UI polish and budget-polish, NOT the second adapter; it **cuts resume** (→ a future stateful `probe_session`). Primary consumer = the **A2A/duet caller**; pipelines + verify/judge are phase-2 consumers. *(Maintainer to confirm both the two-runtime scope and the duet-first consumer ordering.)*

## Open questions

- **OQ1 — Sync cap + async handoff.** Within the stable-envelope contract (D3): what is the sync wait cap, and the exact async handoff (`notify` on done + `read_probe_result(runId)`)? Reconcile with MCP client call-timeout ceilings.
- **OQ2 — Artifact retention / redaction / access-control.** Persisted prompt + event stream + output may carry secrets, customer code, or injected content. Define storage paths, retention windows, max sizes, a redaction strategy, and **who may read a `runId`'s artifacts** before shipping `read_probe_result`.
- **OQ3 — Lifecycle edges, MVP vs deferred.** Cancellation, caller/client disconnect, Bridge restart (orphan-process reaping), a concurrency cap + queueing, idempotency/retry. Which are MVP acceptance gates and which are deferred — explicitly, not by omission.
- **OQ4 — Isolation default for tool-capable probes.** A "usually read-only" probe still touches tools, caches, and generated state. Default to an isolated scratch worktree/overlay for any tool-capable probe, with a verified no-tool/read-only mode as the only un-isolated path?
- **OQ5 — Archetype depth + output-contract form.** Beyond the first two archetypes: are output contracts machine schemas (`verdict`, `confidence`, `artifacts`, parse-error surfacing — needed for pipeline branching) or prose templates? Decide the contract form now even though pipeline nodes ship in phase 2.
- **OQ6 — Sandbox policy strength in v1.** Advisory runtime flags vs a Tachyon enforcement layer wrapping the process (network, home-dir reads, credential-helper access, writes outside the repo). How much enforcement is v1 vs deferred?
