...
3. Render a dense runtime table, not a dashboard of cards. A compact summary strip shows detected runtimes, live
   ...
   and operational fields. At narrow widths the row becomes a labeled two-column detail grid.
...
| Attention/rate limit | `Workspace.attentionOf()` | Preserve state, stale flag, matched throttle source, and parsed reset time. Do not turn an idle state into quota data. |
...
| Bridge health | Current coordinator generation, ledger `bridgeClient`, `durableBoundGeneration()`, and coordinator client state | `ok` requires wired and bound to the current generation. Lower generation is suspect/stale; absent wiring is not wired. Expose generation numbers only in detail/tooltips. |
...
| Workspace | `Workspace.folderName` and `wsHash` | Render folder name; use hash only as an opaque UI key. |

`RuntimeOpsSnapshotV1` is allowlist-by-construction. It excludes terminal contents, raw throttle lines beyond a
bounded display reason, transcript paths, session ids, environment, Bridge tokens, config-home paths, and raw
activity payloads.
...
- **One container, one view.** Runtime and agent data are two levels of the same operational projection. Separate
  panel views would fragment scanning and move each view's actions into separate toolbars.
...
- **Event-driven visible refresh.** Reuse `onViewsChanged("agents")`, add a callback when `ActivityLogManager` appends
  ...
  rereads durable logs while hidden and scales with agent count.
...
- Multi-root and same-name agents can accidentally collapse if keys use only agent name. All keys must include
  workspace hash plus agent name.
