...
The production workspace has five inline agents: `claude`, `claude-orca`, `codex`, `grok` and
`grok-x`. The current planner accepts only literal `codex` and rejects `selfEvolution`, while the
...
`t-088d08`. The cutover is five sequential recoverable transactions with barriers and a checkpoint,
