...
- **D5 — Replace is reversible (Tachyon never destroys user content).** On Replace, the user's original skill is backed up; Remove restores it. The mechanics are OQ2.
...
- **OQ2 — Backup-on-Replace mechanics (the crux of D5).** Where does the user's original skill go so Remove can restore it precisely and a teammate's clone is not corrupted? Candidates: (a) a committed `.tachyon/plugins/.backups/<plugin>/<runtime>/<name>/` + a lockfile pointer; (b) a backup record embedded in the lockfile `skill-dir` target. Must survive a clone (committed), be byte-precise, and degrade safely if the backup is missing at Remove time (warn, don't delete the user's content).
