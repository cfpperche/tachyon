...
- [ ] **Scenario: discover an installed native app**
  ...
  - **Then** stdout lists candidate app records with source, display name, executable path or app id, confidence, and
    launch strategy.
- [ ] **Scenario: explain unresolved app**
  ...
  - **Then** the command fails or returns no candidates with structured search locations checked, not an opaque
    `not-found`.
- [ ] **Scenario: launch and wait for a new native app window**
...
- [ ] **Scenario: ownership proof requires snapshot diff and process tree**
...
- [ ] **Scenario: launcher/single-instance handoff is launched-not-owned**
...
- [ ] **Scenario: launch of an already-running app is not overclaimed**
...
- [ ] **Scenario: conservative native cleanup**
  ...
  - **Then** dry-run reports what would close; cleanup sends `WM_CLOSE`, revalidates identity first, and never kills the
    process by default.
- [ ] **Scenario: native app exits before cleanup**
...
- [ ] **Scenario: unsaved-work prompt is not killed**
...
- [ ] **Scenario: user app/window survives cleanup**
...
- [ ] **Scenario: resolver supports explicit path escape hatch**
...
- [ ] **Scenario: resolver is inspectable before mutation**
...
- [ ] **Scenario: ambiguous resolver refuses launch**
...
- [ ] **Scenario: uninstall/setup/update shortcuts are denied**
...
- [ ] **Scenario: hostile app query is inert data**
...
- [ ] **Scenario: wait-window timeout is safe**
...
- [ ] The resolver searches at least: explicit path, built-in aliases, Windows App Paths registry, Start Menu shortcuts,
  `PATH`, and Program Files common locations.
- [ ] Resolver explain output has a stable JSON schema: `query`, `chosen`, `refused_reason`, and `candidates[]` with
  `source`, `display_name`, `target`, `arguments`, `match_kind`, `score`, `confidence`, and `denied_reason`.
- [ ] The resolver never executes arbitrary shell text from shortcuts or registry values; launch arguments are structured.
- [ ] Browser candidates covered by spec 336 are routed to the safer browser/session path or refused with a pointer to
  `open-url`; they do not use generic native ownership.
- [ ] MSIX/UWP/AUMID candidates may be discoverable/launchable, but are always `owned=false` in v1 because
  `ApplicationFrameHost.exe` makes window ownership unsafe.
- [ ] UAC-elevated apps are launched-not-owned unless identity checks can run at matching integrity.
- [ ] The CLI keeps JSON-only stdout and stable exit codes.
- [ ] Docs explain that app launch can open recent files/projects and that user consent covers that desktop mutation.
