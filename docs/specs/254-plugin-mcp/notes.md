...
Decision: commit Step 4 (engine I/O is sound + the actionable hardening is folded); the two dispositioned items are an accepted systemic boundary + Step-5 work. Suite 1288 green. Committed `a6cbee1`.

## Step 5 — consent VM + drawer MCP section + double-confirm (closes Step-4 #6)

- **consentViewModel.ts:** `ConsentMcp` (name, transport, `detail`=command+args | url, `env`=required-env names via mcpRequiredEnv, runtimes) + `ConsentMcpCollision` (server, runtime, `key`=`${runtime} ${ref}` = the applyInstall mcpDecisions key) + `mcpFrom()` (groups a server on both runtimes into one row); wired into buildInstallConsent + buildUpdateConsent. **`requiresMcpConfirm: true` whenever the plan writes ANY MCP server** (OQ5).
- **App.tsx ConsentDrawer:** an MCP servers section (the security surface — exact command/url + env refs), per-collision Keep/Replace (default Keep), and the **mandatory ack shown for EVERY MCP install** (`requiresMcpConfirm`, not just Replace — OQ5). `blocked` includes `requiresMcpConfirm && !mcpAck`, so Install is disabled until acked. `key={consent.token}` already remounts the drawer per consent (no stale ack/decision leak — the spec-251 Step-4 fix already in place).
- **main.tsx + PluginsPanel.ts:** `mcpDecisions` threaded confirm → confirmOp → applyInstall/applyUpdate (the engine fail-closes on an undecided collision — the real net).
- Tests: +3 consent-VM (MCP section + env + requiresMcpConfirm; collision key; omitted when none). UI proven by the real built bundle — new render harness `harness/plugins-consent.tsx` (+ render.mjs entry); `out/plugins-consent-{dark,light}.png` show the MCP section, the `db (claude)` collision Keep/Replace, and the ack-gated (disabled) Install button.

Post: full suite 1290 green; tsc ×2 + engine-boundary + esbuild clean.

### Codex Step-5 review (SHIP-WITH-CHANGES → folded)

- **[HIGH] OQ5 was UI-only.** The drawer disabled Install until the ack, but the ENGINE didn't enforce it — a raw `confirm` message / direct engine caller could install non-colliding MCP servers without the ack. Fixed: `mcpConfirmed` threads dispatch → inbound → confirmOp → applyInstall/applyUpdate, and **`applyInstall` FAIL-CLOSES**: `fresh.mcpTargets.length > 0 && opts.mcpConfirmed !== true` → refuse (placed before collision/lost-update checks). The drawer passes `mcpAck` as `mcpConfirmed`. Now the double-confirm is a real engine gate (like the collision fail-closed), not just a disabled button. Test added (refuse without / install with). Test helper `install()` + the MCP I/O tests pass `mcpConfirmed:true` (the drawer stand-in).
- **[LOW] UI-contract tests.** The engine gate test covers the security-critical path; the disabled-before-ack / enabled-after-ack / ack-reset UI behavior is proven by the real built bundle (render harness `plugins-consent`), per the codebase's "UI via real bundle" convention for the vscode-webview layer.

Post-fold: full suite 1291 green; tsc ×2 + engine-boundary + esbuild clean. Step 5 ready to commit (closes the Step-4 #6).
