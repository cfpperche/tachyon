...
**Status detail:** SHIPPED (v1) — all 6 steps landed, each codex-reviewed (Step 4 took a full security round). MCP is the third plugin capability after hooks (250) + skills (251). See `tasks.md` § Closure for the capability summary, security model, and deferred v1 limits. Live-dogfooded against `tachyon-plugin-example@v4.0.0`.
**Follows:** [spec 250](../250-tachyon-plugin-system/) (the engine; already reserves the `mcp-server` `TargetKind` in the lockfile) and [spec 251](../251-plugin-skills/) (the neutral-payload + Keep/Replace + declare-and-skip pattern this mirrors).
**Surface:** the plugin manifest/payload (a neutral MCP-server declaration), the materialization engine (an `mcp-server` install/remove path), the per-runtime adapters (claude `.mcp.json` + codex `.codex/config.toml`), and the consent drawer (an MCP section — the highest-risk capability yet).
**UI impact:** flow (the consent drawer gains an MCP section + per-collision Keep/Replace; installed cards may surface an MCP capability).
