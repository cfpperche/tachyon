...
**Status:** DESIGN AGREED — codex-debated (verdict REVISE, folded) + maintainer sign-off (OQ5 = double-confirm every MCP install). Ready for `tasks.md` decomposition; the codex required-changes checklist is the plan gate. · **Follows:** [spec 250](../250-tachyon-plugin-system/) (the engine; already reserves the `mcp-server` `TargetKind` in the lockfile) and [spec 251](../251-plugin-skills/) (the neutral-payload + Keep/Replace + declare-and-skip pattern this mirrors). · **Surface:** the plugin manifest/payload (a neutral MCP-server declaration), the materialization engine (an `mcp-server` install/remove path), the per-runtime adapters (claude `.mcp.json` + codex `.codex/config.toml`), and the consent drawer (an MCP section — the highest-risk capability yet). · **UI impact:** flow (the consent drawer gains an MCP section + per-collision Keep/Replace; installed cards may surface an MCP capability).
