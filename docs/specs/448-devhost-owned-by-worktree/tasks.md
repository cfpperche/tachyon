...
- [ ] **T1 — prove the load-bearing assumption (plan R1).** From a linked worktree, arm a dev-host by
...
- [ ] **T2 — flatten the layout.** `pathsOf(checkoutRoot)` returns `dev-host/{extension,workspace,runtime,meta.json,user-data,extensions,tmux,cache,profile-home}` with no `slots/` segment and no slot id parameter.
- [ ] **T3 — root the dev-host in the current checkout.** `point()` builds under the checkout it runs
...
- [ ] **T4 — retire the flags loudly.** `--owner` and `--slot` raise an error naming the replacement
      flow (no window, plan D4). `--worktree` becomes optional, defaulting to the current checkout.
- [ ] **T5 — delete the slot/active machinery.** `SLOTS_SUBDIR`, `ACTIVE_LINK`, `normalizeSlotId`,
...
- [ ] **T6 — make `launch.json` static (plan D3).** Delete `ensurePortableLaunchConfig`,
...
- [ ] **T7 — retarget the headless harnesses (plan D5).** `resolvePointerSlotRoot` in
...
- [ ] **T8 — drop `--owner`/`--slot` pass-through** in `scripts/dev-host/lane.mjs` and `cli.sh`.
- [ ] **T9 — rewrite the tests (plan R4, the bulk of the work).** Delete the `t-efe06d: multi-slot
...
- [ ] **T10 — structural guard.** New test banning `.tachyon/dev-host/active` and any `slots/` layout
      across `scripts/`, `src/`, `test/`, `docs/`. Prove it non-vacuous: reintroduce a reference,
      watch it fail, remove it.
- [ ] **T11 — runbook.** `docs/runbooks/dev-host.md`: one flow (arm in your worktree → open VS Code
...
- [ ] **T12 — migration residue.** Restore the primary checkout's `.vscode/launch.json` to its
...
- [ ] Two worktrees each arm a dev-host and run an EDH concurrently, neither touching the other, with no slot identifier anywhere → spec scenario 1.
- [ ] `git status --porcelain` is byte-identical before and after an arm, in every checkout → spec scenario 2.
- [ ] Removing a worktree removes its dev-host with it; no directory is left in the primary monorepo → spec scenario 3.
- [ ] The headless harness invoked inside a worktree resolves that worktree's dev-host, with no `active` indirection → spec scenario 4.
- [ ] `--owner` / `--slot` exit non-zero with a message naming the replacement → spec scenario 5.
- [ ] `.vscode/launch.json` is written by no script and is identical across checkouts → spec static fact 1.
- [ ] The structural guard fails when `active`/`slots/` is reintroduced → spec static fact 2.
...
**Dogfood:** `npm run dogfood:dev-host -- point-status`
