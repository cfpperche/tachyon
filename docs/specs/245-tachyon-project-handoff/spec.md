...
- **G — agent-driven distill (0.33.0, post-dogfood).** Human raw-edit distillation is too much friction; the model
  is "agent distills, human curates." `get_project_handoff` now returns the pending NOTE ROWS + `pending_through`
  (the distill watermark); an agent folds them into a rewritten body, the human approves in chat, the agent calls
  `set_project_handoff(content, expected_revision, distilled_through=pending_through)`. A new `meta.distilled_through`
  watermark (separate from `updated_at`) drives pending — so a note appended DURING a distill isn't silently
  dropped (codex BLOCK). `advanceWatermark` is forward-only + strict-canonical-ISO (codex ×3: race → watermark →
  doc+regress guard → Date.parse-too-lax → extended-year regex). v1.1 still deferred: a panel "Distill" button +
  the R2 state-targeted nudge.
...
## Closure
**Closure:** Shipped in 0.32.0. First migration of an Agent0 harness mechanism into the Tachyon product (the "minimal pollution / Tachyon-as-harness" thesis). Two-lane model (codex-debated): canonical `.tachyon/HANDOFF.md` (human-curated, CAS) + `handoff-notes.jsonl` (append-only, any agent). Increments A–E: pure store, 3 Bridge tools, SessionStart pointer + per-workspace idle append-nudge (`settings.handoff.nudgeEvery`), config, and a read-only editor webview panel opened by a per-folder sidebar button (built via `/frontend-designer`, reusing the Activity panel's sanitized `MarkdownView` + `--vscode-*` tokens). Codex: design debate (two-lane) → impl review SHIP-WITH-CHANGES on the engine (atomic write + cwd guard, folded into 243-era infra) and on the panel (P1 use sanitized MarkdownView not raw renderMarkdownHtml; P2 reuse engine `pendingNotes` — both folded). 30 new tests (929 total). **Increment F (0.32.1):** the append-nudge gained a per-agent activity-lag gate (`shouldRemindHandoff`, anchor advances on nudge+append) so an already-logged/idle agent isn't re-nudged for the same work — codex SHIP, +1 test (930). **Increment G (0.33.0):** agent-driven distill — `get` exposes the pending note rows + `pending_through` watermark, `set` takes `distilled_through`; new `meta.distilled_through` (separate from `updated_at`) so a concurrent append during a distill isn't dropped; `advanceWatermark` forward-only + strict-canonical-ISO. Codex: BLOCK (append-during-distill) → fix → 2 more folds (doc + regress/format guard) → SHIP. (932 total.) Deferred to v1.1: `mark_handoff_notes_distilled` + assisted distill; nudge R2 (state-targeted: nudge the owner to distill vs agents to append). **Live panel/append/multi-root + nudge behavior is the user's EDH gate.**
