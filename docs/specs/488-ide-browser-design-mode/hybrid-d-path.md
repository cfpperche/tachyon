...
| **D3** | **F1 — pane-text reply protocol** | Product risk was losing replies before the tool path landed live | **Executed by `t-45b266` after panel-land dogfood on the new webview.** `design_mode_chat_reply` is the only path for the supported Claude/Codex/Grok scope. |
