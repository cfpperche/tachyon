...
- [x] **Scenario: create change worktree**
...
- [x] **Scenario: fail-closed occupied**
  - **Given** a worktree occupied by a live agent cwd
  - **When** remove is requested
  - **Then** remove fails without deleting
- [x] GitDelivery prune worktree removal goes through WorktreeManager/engine, not ad-hoc argv only
- [x] Unit tests cover registry pure ops + service create/list/remove
...
Merged to **main** 2026-07-16 from `grok/managed-worktree-registry` (R4 ACCEPT-WITH-FIXES + P2s + cookbook).
Focused suite 170 pass post-merge. Installed multi-root dogfood deferred → status remains `shipped-partial`.
