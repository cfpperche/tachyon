...
- 2026-07-24 — **t-44dfb6:** probe (Claude Sonnet adversarial) → REDESIGN-narrow: implement concurrent one-per-kind after SSE `tab.command` isolation + `hasPairedKind("browser")` for tab tools. Landed: `Map<kind,session>`, `replacedSessionToken` same-kind only, Control `deviceId` unpair, tests for co-exist + token filter.
- 2026-07-24 — **t-bd281f:** expanded cookbook Privacy/A2HS + companion monorepo `PRIVACY.md` / `apps/mobile/README` (Tailscale path, unpair hygiene).
