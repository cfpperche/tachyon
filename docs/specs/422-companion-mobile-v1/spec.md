...
**Board:** umbrella `t-af2c9b` · product card `t-fe52f0` (frente 2) · research `t-619157` done · parent browser 414 shipped  
**Kind:** product SDD (Companion mobile external shell) — PWA + LAN pair + narrow fleet controls  
...
This spec defines **Tachyon Companion Mobile v1**: a **local-first web app / installable PWA** (`apps/mobile` in `tachyon-companion`) that pairs with the **same engine** as a thin shell — observe roster/attention, send a bounded prompt, resolve host-authoritative approvals — over an **opt-in LAN** reachability path. The phone is not a second orchestrator, not Mission Control cloud, and not a browser-tab automation client.

**Done for product v1:** on a trusted Wi‑Fi, a human enables LAN companion access, scans a QR (base URL + pair code) from Control, sees agents that need input, sends one short prompt or Accept/Deny an approval, and unpairs — without store apps, cloud relay, terminal scrollback, or Runtime API unificação.
...
| M2 | Network | Opt-in LAN bind + QR pair (`baseUrl` + pair code + protocolVersion) |
| M3 | Protocol | Reuse `/companion/v1` + existing pair; `client.kind=mobile` |
| M4 | Concurrent devices | Prefer **browser + mobile** co-exist if low cost; else document last-pair-wins |
...
[Engine Bridge listener — companion routes]
    loopback default; optional LAN when settings.companion.lanAccess
    /companion/v1/pair|status|agents|prompt|approvals|events
    |
    v
