...
- [ ] 1. **Ledger schema split** (`src/resume/SessionLedger.ts`): `SessionRecord` =
...
- [ ] 2. **Record every ad-hoc on spawn, after success** (`AgentManager.spawn`): write
...
- [ ] 3. **`rehydrateFromLedger()`** (`AgentManager`): repopulate `adhoc` from records
...
- [ ] 4. **Activation wiring** (`Workspace`): `rehydrateFromLedger()` before
      `planResume(...)`; restart of a re-discovered ad-hoc succeeds (AgentManager test).
- [ ] 4b. **Filter resume on `isResumable`** — `planResume` offers, the Sidebar
...
- [ ] 4c. **Lifecycle honesty**: kill/dismiss of an ad-hoc **removes its ledger row**;
...
- [ ] 5. **Promote** (`tachyon.promoteAgentItem`): **extend `YamlConfigEditor.addAgent`
...
- [ ] 6. **Sidebar/menu**: a contextValue distinguishing ad-hoc from declared agents;
      "Save to tachyon.yml" (inline/context, palette-hidden) only on ad-hoc.
- [ ] 7. **Docs**: README note on ad-hoc persistence + promotion; tool/MCP docs if
