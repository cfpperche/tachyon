...
- [x] 1. **Pure `src/worktree/review.ts`**: `ChangedFile` type ({status, path, binary?});
...
- [x] 2. **WorktreeManager**: `changedFiles(cwd, baseRef)` = `git diff --name-status <baseRef>`
...
- [x] 3. **Extension content provider + command**: register a `TextDocumentContentProvider`
...
- [x] 4. **package.json + nls**: `reviewWorktreeItem` command (`$(git-compare)` icon) + a
      `view/item/context` entry `when viewItem =~ /worktree/`; titles in en + pt-br.
- [x] 5. **Docs**: README worktree section — a "Review changes (native diff, base ↔ current)" line.
- [x] 6. **Live smoke** (EDH): a worktree agent with committed + uncommitted + an untracked
