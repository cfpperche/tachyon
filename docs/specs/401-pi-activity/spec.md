...
Add a pure, stateful Pi transcript normalizer and wire it into the existing bounded, durable `ActivityLogWriter`. Activity must tail only the exact transcript already attributed by SDD 399/400, preserve source provenance and runtime semantics, avoid rendering the Tachyon primer as a human turn, correlate tool results without inventing success, and degrade safely on malformed or unknown Pi entries.
...
- Parsing arbitrary sibling/newest Pi transcripts; SDD 399/400 exact attribution remains authoritative.
