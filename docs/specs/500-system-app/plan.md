...
_Prototypes and durable evidence are opt-in. When this spec needs them, keep them inside `docs/specs/500-system-app/` (for example `prototypes/` or `evidence/`) unless a non-empty `**Artifact-Location-Opt-Out:** <reason>` documents why the artifact has a different owner._
