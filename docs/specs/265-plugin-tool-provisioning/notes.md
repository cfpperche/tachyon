...
#### TASK 10 — codex design review (SHIP-WITH-CHANGES → folded, pre-build)

Adversarial codex review of the transactional-provisioning plan (transcript: Agent0 `.agent0/.runtime-state/codex-exec/*task-10*` / the `…task10…` run). Verdict SHIP-WITH-CHANGES, one BLOCK; folded:
- **B (BLOCK — the #1 fix) — launcher resolution must be PLUGIN-SCOPED.** Today `resolveToolForLaunch(toolName)` scans ALL plugins' `tools[]` and fails on ambiguity — so a later plugin installing a same-named tool can hijack/break an existing hook. Fix (corrects shipped task 7): the launcher takes `<pluginName> <toolName>` and resolves ONLY `plugins[pluginName].tools[]`; `${tool:}` materializes to `_tachyon-tool <pluginName> <toolName>`; no cross-plugin tool refs in v1.
- **A — lockfile commit is also a launcher-resolution activation point** for already-active hooks (an update): document it as partial-but-safe; plugin-scoping (B) contains it. Crash ordering otherwise safe (binary-live-before-lockfile → orphan litter GC'd by physical refcount; hooks activate last + fail-closed).
- **C — workspace-global launcher**: record its integrity in a workspace-level managed record (not only mutable `.tachyon/bin`); make `materializeLauncher` atomic under the install lock (two installs can race today); **rehydrate must RE-RESOLVE Node** (the baked absolute nodePath can't be trusted on clone/CI/moved-workspace).
- **D — finalUrl exact-match is too brittle** (CDNs ignore Range / sign URLs / route per-region). Bind consent to `declaredUrl + artifactSha256 + binSha256 + resolvedPlatform`; treat finalUrl as recorded provenance with a normalized equivalence (abort on scheme downgrade / declaredUrl change / redirect off the consented host policy / checksum mismatch — NOT on a benign signed-query change when the bytes match the pinned sha256). → adjusts the task-9 fingerprint to DROP finalUrl from the hard binding.
- **E — slice task 10** (it's too big to land atomically): **10a** inert plumbing (transaction journal + startup GC + launcher integrity record + a `${tool:}` parser that rejects invalid/unsupported placeholders; no activation) → **10b** provisioning commit path (provision fetched tools + smoke + atomic launcher write + `PluginLock.tools[]` + rollback/orphan-GC; no `${tool:}` activation) → **10c** scoped hook activation (replace whole-token `${tool:plugin/name}` → scoped launcher invocation, leaves written last, update/crash tests) → **10d** removal/rehydrate (= task 11: physical-refcount delete, launcher removal when no refs, clone/CI rehydrate re-resolving Node + tools).

Single most important: **plugin-scope launcher resolution BEFORE any activation** — applied now as a task-7 correction.
