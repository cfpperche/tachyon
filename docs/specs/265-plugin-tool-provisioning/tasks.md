...
- [ ] 10. **engine apply — transactional provisioning + `${tool:}` resolution.** SLICED per codex task-10 review (E) into fail-closed boundaries:
  - [ ] 10a — inert plumbing: transaction journal `.tachyon/transactions/<id>/` + startup GC + workspace-level launcher integrity record + a `${tool:plugin/name}` PARSER that rejects invalid/unsupported/cross-plugin placeholders. No activation.
  - [ ] 10b — provisioning commit path: gather toolPlan + re-resolve redirects (H4: bind `declaredUrl+artifactSha256+binSha256+platform`, finalUrl=provenance), `toolConfirmed` ack gate, provision fetched tools → smoke → atomic `materializeLauncher` (trust-checked Node) → write `PluginLock.tools[]` → rollback/orphan-GC by physical refcount. No `${tool:}` activation.
  - [ ] 10c — scoped hook activation: replace whole-token `${tool:plugin/name}` → `_tachyon-tool <plugin> <tool>` (plugin-scoped, codex B — launcher ALREADY corrected), leaves written LAST, update/crash tests.
