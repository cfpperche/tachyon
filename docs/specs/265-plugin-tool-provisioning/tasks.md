...
- [x] 11. **engine remove + rehydrate.** `applyRemove` → `removeProvisionedTools`: deletes each FETCHED content-addressed binary no surviving plugin references (physical refcount, H7), never a host-provided one; removes the launcher + drops `Lockfile.launcher` when no plugin has tools left. `rehydrateTools(workspaceRoot, …)`: re-provisions FETCHED tools from the committed lockfile (clone/CI — `.tachyon/bin` gitignored), idempotent, RE-RESOLVES Node, re-materializes the launcher; never a silent fetch. e2e: remove deletes binary+launcher; rehydrate restores both after `.tachyon/bin` is wiped.
