# 499 — task-attempt-ledger — plan

_Drafted from `spec.md` on 2026-08-09. The approach, not the steps (those go in `tasks.md`)._
...
_Prototypes and durable evidence are opt-in. When this spec needs them, keep them inside `docs/specs/499-task-attempt-ledger/` (for example `prototypes/` or `evidence/`) unless a non-empty `**Artifact-Location-Opt-Out:** <reason>` documents why the artifact has a different owner._
