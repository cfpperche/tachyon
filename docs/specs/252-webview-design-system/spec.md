...
**Status detail:** SHIPPED 2026-06-24 (all 6 webviews migrated; codex-reviewed; verified dark+light).
**Surface:** a shared `src/webview/shared/design-system.css` (copied to `dist/webview/`, linked by every host `html()`), then a migration of all 6 webviews onto it.
**UI impact:** ui (every Tachyon webview re-skins to one system; no behavior change).
**Verify:** each migrated panel is screenshotted under BOTH a dark and a light VS Code theme to prove it follows the user's theme.
