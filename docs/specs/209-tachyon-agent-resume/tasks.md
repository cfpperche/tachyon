...
- [x] 1. `src/resume/adapters.ts` — `SessionAdapter` interface + pure adapters for
...
- [x] 2. `src/resume/SessionLedger.ts` — load/save/merge `.tachyon/sessions.json`
      (`agentName -> {runtime, sessionId, cwd, spawnFlags, lastSeen}`); tolerant of
      missing/corrupt file. Pure; unit-tested.
- [x] 3. Spawn wiring (`AgentManager`/`Workspace`): on spawn, pick the adapter; if
...
- [x] 4. Activation resume (`Workspace` activate): classify each ledger agent
...
- [x] 5. UX: `tachyon.resumeAgent` + `tachyon.resumeAll` commands, a sidebar
...
- [x] 6. Capture-runtime id resolvers (real-CLI-shaped): codex `session_meta.cwd`,
      opencode `storage/project` worktree→`ses_*`, qwen in-cwd, continue JSON. Pure
      parsers over fixture files; unit-tested.
- [~] 7. (live claude+codex smoke DONE; xvfb reopen-resume deferred)
  - [ ] 7-orig: Per-runtime real-CLI smoke tests (skipped when the binary is absent, like
