...
- [ ] **T4.3a (honesty limit — P1 review finding 5)** — In **Tier A this precondition is INTEGRITY-checked, not AUTHENTICATED**: the verification store is same-UID-writable and its `integrityHash` is an unkeyed sha256, so a determined agent can forge a `verdict:accept` record. It catches corruption / stale / wrong-commit records, NOT adversarial forgery. The approval payload and any surfacing MUST word it as a **recorded claim the off-machine consumer corroborates**, never as proof the commit passed the gate; the T1.5 no-overclaim scanner covers this wording. It becomes a real gate only under Tier E (T6.1), where the agent principal cannot write the store.
