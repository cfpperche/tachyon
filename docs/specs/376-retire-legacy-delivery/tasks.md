...
- [ ] **T1 — Freeze the canonical-only contract and state-retirement fixture.** Capture current canonical
...
- [ ] **T2 — Remove product selection and old public entry points.** Delete `settings.delivery`, obsolete
...
- [ ] **T3 — Remove legacy models and verifier adaptation.** Refactor verification and primer/re-anchor to consume
...
- [ ] **T4 — Enforce projection-only GitDelivery.** Require immutable `deliveryId`, route all creation/mutation
...
- [ ] **T5 — Ship the explicit retirement action.** Implement preview + confirmed archive/apply with digest,
...
- [ ] No current source/tool/config path can select or execute the legacy delivery lifecycle.
- [ ] Canonical creation, sequential join, verification, projection mutation, reload, and compensation tests pass.
- [ ] Retirement preview/apply proves byte-stable canonical state and Git state, plus idempotent recovery.
- [ ] Removed lifecycle symbols and schema fields are absent outside the raw retirement module/fixtures
  (historical specs exempt).
- [ ] Installed happy-path dogfood uses one Delivery/worktree and emits no legacy artifact.
- [ ] `npm run typecheck`, `git diff --check`, and `npm run verify:full:quiet` pass at final HEAD.
