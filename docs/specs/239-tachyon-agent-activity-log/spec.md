...
## Post-ship follow-up — orphaned ephemeral logs (pin p-4dadd3, 2026-06-22)

**Found during spec 245 dogfood.** Two by-design NON-PERSISTENT agent classes get a durable `.tachyon/activity/<agent>.jsonl` while they run, then are reaped from the ledger on a clean exit — leaving an orphaned, **unreachable** log (the Activity panel only opens per LIVE sidebar row, so there's no affordance to view a log whose row is gone): (1) **clean-exit ad-hoc one-shots** (reaped in `AgentManager.list()`, spec-211 F6), and (2) **inline pipeline `cmd:` nodes** `pl-<runId>-<nodeId>` (torn down in `Workspace.dismissNode`). With keep-all retention (D5) these accumulate per run, growing `.tachyon/activity/` with files nobody can open.

**Options weighed:** (a) don't keep the log for non-persisted ephemerals; (b) keep + add a history/archive view; (c) leave as on-disk forensics.

**Decision — (a).** The log is the durable archive **of a durable agent** (D1/D5), keyed by the tachyon agent. An agent that is reaped by design is not a durable agent, so **the log's lifecycle must equal the ledger row's: born with the row, deleted with it.** (b) is rejected as speculative-observability harness-drift — surfaced once, no rule-of-three demand, and the pin itself flagged the risk. (c) is rejected — it leaves unbounded, unreachable orphans that contradict the "archive of an agent" model. Live observability is unchanged: while the ephemeral runs it has a row and the panel renders normally; only post-death persistence is dropped.

- **Pipeline-run forensics deferral.** Deep post-mortem of a pipeline node belongs to a future **run-scoped** re-run/history feature (Tier B, pin p-cbcc94 — deferred by cost). If that lands with real demand, persist node activity attached to the RUN, not as a per-agent orphan. (a) forecloses nothing here.
- **Implementation.** `deleteActivityLog(dir, agent)` in `src/activity/logStore.ts` drops the `.jsonl` + `.state.json` sidecar (best-effort; shared content-addressed `blobs/` left for separate GC). Called at the agent's **true end-of-life**, not the ledger-row reap:
  - **`AgentManager.dismissAdhoc()`** — NOT `list()`'s clean-exit reap. `remain-on-exit` keeps a clean-exit one-shot as a viewable **dead pane** that still offers "Activity" in postmortem (`sidebar/actions.ts`), surviving reload, until the user dismisses it; deleting at the `list()` reap would break that working view (codex MAJOR #1). Deletion at `dismissAdhoc` (kill of the dead pane) is the point the row AND pane both vanish → genuinely unreachable. NOT in `forgetAdhoc` (promotion to a declared tachyon.yml agent KEEPS the log).
  - **`Workspace.dismissNode`** gated on `!def?.agent` (an inline `cmd:` node vanishes → delete; a DECLARED `agent:` node reverts to a persistent stopped agent → KEEPS its log).
  - **Race guard in `ActivityLogManager.tick()`** — the ~2s background writer snapshots `ledger.all()` then awaits `transcriptPathOf`; a dismiss landing mid-await could let the stale tick `poll` and re-append (fresh `appendFileSync`, no held fd), resurrecting the just-deleted orphan (codex MAJOR #2). Fixed by re-checking the row is still present+resumable immediately before `poll`; if gone, drop the writer and skip.
  - Tests: `test/unit/logStore.test.ts` (+3), `agentManager.test.ts` (dismissAdhoc deletes the log), `activityLogManager.test.ts` (mid-tick dismiss race — no resurrection).
