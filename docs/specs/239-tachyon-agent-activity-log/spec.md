...
- **Implementation.** `deleteActivityLog(dir, agent)` in `src/activity/logStore.ts` drops the `.jsonl` + `.state.json` sidecar (best-effort; shared content-addressed `blobs/` left for separate GC). Called at the two existing reap points: `AgentManager.list()` clean-exit reap, and `Workspace.dismissNode` gated on `!def?.agent` (a DECLARED `agent:` node reverts to a persistent stopped agent and KEEPS its log). Tests: `test/unit/logStore.test.ts` (+3).
