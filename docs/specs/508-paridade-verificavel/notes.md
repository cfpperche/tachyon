...
|---|---|---|
| Native lane suppression (§3.1) | **não derivável** — permanece narrativa | `t-4c3d90` devolveu o registro a `packages/engine/src/runtime/nativeLaneSuppression.ts` e o host passa a chamar `isNativeSuppressionConfirmed`. Continua sem célula: o veredito é o campo `evidence` escrito à mão, então derivá-lo compararia o registro consigo mesmo (a tautologia da row 15). Memory disable já é a row 15; contar a conjunção seria contar essa row duas vezes. |
