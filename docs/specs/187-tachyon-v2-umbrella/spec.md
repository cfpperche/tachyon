...
| F7 | **Publishing kit** — LICENSE, marketplace icon PNG, categories/keywords/galleryBanner, `vsce package` dry-run clean | Product maturity | S | — | **implemented** (LICENSE + icon.png + AI category + banner; vsix packages clean, 16 files; `vsce publish` is the human step w/ a marketplace PAT) |
...
| F13 | **Agent CRUD from the UI** — New/Clone/Delete/Edit on the sidebar, mutating tachyon.yml via comment-preserving yaml Document API (file stays source of truth). Rename left this surface in t-4662e9: it rewrote only the `tachyon.yml` entry while the Agent Form runs the canonical profile lifecycle, so the Agent Form is the single rename surface | User request (pre-F9 foundation UX) | S/M | 193-tachyon-agent-crud-ui | **implemented** (shipped 2026-06-10, 25345e3) |
...
| F19 | **wait_for_agent** — event-driven long-poll tool (Waiters off monitor transitions); completes the delegation cycle spawn→wait→read→kill | User delegation scenario + sentinel websocket insight | S | 198-tachyon-wait-for-agent | **implemented** (shipped 2026-06-10, d846707) |
| F20 | **tmux control-mode engine** — replace capture-pane polling with `tmux -C` event stream under the monitors (instant detection, less subprocess churn; CPU sampling stays); waiters/F19 benefit untouched | sentinel architecture (layer-1) | M/L | 202-tachyon-control-mode | **implemented** (shipped 2026-06-10; crash detection 3s+ → ~1s, zero subprocess churn) |
...
| F23 | **Schedules** — `schedules:` in tachyon.yml (`every:`/`at:`, action = run_command/run_runbook/spawn_agent); fires only while the workspace is open (honest scope, watch-restart semantics), optional `catchUp`; Schedules sidebar view (next/last run, pause, edit/delete) + Agent Studio Schedule tab (spec 207); agents may `propose_schedule` via the Bridge but proposals are INVALID until the human approves (approve = written into tachyon.yml; pending lives in `.tachyon/`) | User request 2026-06-11 (runtime-neutral scheduling; Claude has /schedule, others have nothing) | M | 206-tachyon-schedules | **implemented** (shipped 2026-06-11, 0.6.0) |
