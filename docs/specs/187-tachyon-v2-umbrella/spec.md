...
| F23 | **Schedules** — `schedules:` in tachyon.yml (`every:`/`at:`, action = run_command/run_runbook/spawn_agent); fires only while the workspace is open (honest scope, watch-restart semantics), optional `catchUp`; sidebar section (next-run, pause); agents may `propose_schedule` via the Bridge but proposals are INVALID until the human approves (approve = written into tachyon.yml; pending lives in `.tachyon/`) | User request 2026-06-11 (runtime-neutral scheduling; Claude has /schedule, others have nothing) | M | — | pending (briefed + design agreed in session; awaiting go) |
