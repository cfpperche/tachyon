...
| F20 | **tmux control-mode engine** — replace capture-pane polling with `tmux -C` event stream under the monitors (instant detection, less subprocess churn; CPU sampling stays); waiters/F19 benefit untouched | sentinel architecture (layer-1) | M/L | — | pending (agreed next after F19; seam: monitor transitions) |
...
| F21 | **Runbooks** — named step sequences (steps reference commands or inline shell), exit-code gated, failed pane kept + rest skipped; run_runbook Bridge tool | sentinel parity (user request) | M | 200-tachyon-runbooks | **implemented** (shipped 2026-06-10, 35e0c73) |
