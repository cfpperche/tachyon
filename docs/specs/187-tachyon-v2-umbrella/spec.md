...
| F20 | **tmux control-mode engine** — replace capture-pane polling with `tmux -C` event stream under the monitors (instant detection, less subprocess churn; CPU sampling stays); waiters/F19 benefit untouched | sentinel architecture (layer-1) | M/L | 202-tachyon-control-mode | pending (**planned** 2026-06-10 — spec+plan drafted, spike-gated; awaiting go) |
