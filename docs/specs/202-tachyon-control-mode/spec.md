...
- [ ] **Scenario: command channel replaces subprocess churn**
...
- [ ] **Scenario: event-driven lifecycle**
...
- [ ] **Scenario: degraded mode is safe**
...
- [ ] **Scenario: anchor session is invisible**
...
- [ ] Real-tmux suite covers the client end-to-end (throwaway socket): connect, exec round-trip, octal-unescaped `%output`, subscription firing on pane death, reconnect after kill-server
- [ ] Attention monitor SEMANTICS unchanged (capture+stability state machine stays); its captures ride the persistent channel
- [ ] No version-notice churn: no tool schema change (patch bump only)
...
- [ ] Subscription format-loop nesting (`#{S:…#{W:…#{P:…}}}`) across tmux versions ≥3.2 — task 1 spikes this on the real socket before anything builds on it.
