...
SDD 398 proved that a Tachyon-managed Pi receives its primer, authenticated identity and Bridge tools, but the engine still classifies Pi as non-resumable. A stopped/crashed Pi therefore cannot advertise or execute Tachyon's exact-session Resume flow even though Pi exposes caller-selected `--session-id` and exact `--session <id>` primitives with durable JSONL transcripts.
