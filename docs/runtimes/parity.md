...
|---|---|---|---|---|---|:---:|
| Claude | selected auth/bootstrap only | explicit workspace policy projects closed scalar keys; selected owner-captured skills/hooks/MCP require exact Claude grants; ambient `settings.local.json`, plugin roots and unselected workspace tooling remain excluded | private `CLAUDE_CONFIG_DIR`; generated settings+hooks, captured skill tree, strict selected-MCP+Bridge config, and manifest-last provenance | credential symlink plus onboarding markers; auth is not profile-authored | fresh/restart/resume regenerate the same selected generation and remove stale tooling (`t-debbfe`, `t-2f37e7`, SDD 460/462, 2026-07-25); fork remains explicitly unavailable for isolated Claude homes | ~ |
