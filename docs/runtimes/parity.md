...
|---|---|---|---|---|---|:---:|
| Claude | selected auth/bootstrap only | explicit workspace policy can project closed `permissions`, `interface`, and `featureFlags` keys; unselected/unknown keys fail closed; `settings.local.json`, skills, hooks, plugins and workspace MCP are excluded | private `CLAUDE_CONFIG_DIR`, generated `settings.json`, exact trust, and strict Bridge-only MCP config | credential symlink plus onboarding markers; auth is not profile-authored | fresh/restart/resume use the same every-launch materializer and remove stale settings/tooling (`t-debbfe`, SDD 460, 2026-07-25); fork is explicitly unavailable for isolated Claude homes; capability projection remains unsupported | ~ |
