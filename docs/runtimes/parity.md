...
| Fork | `--session-id <new> --fork <exact-source-path>` | positive extension ownership + no-follow source validation + native `forkCommand` | **✓** SDD 404 unit + real + human Dev Host A→B→independent Resume pass |
