...
ⁿ **Grok fork** is native for legacy/ad-hoc agents, and refused for a CANONICAL profile: the private-home
fork path covers only Pi and Claude, so the fork would carry neither the projected config nor the source
transcript (`t-26f508`).
