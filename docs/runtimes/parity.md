...
| Codex | canonical policy can project reviewed global scalar families; auth stays external | canonical policy can project reviewed workspace scalar families; unselected keys fail closed | private `CODEX_HOME`; atomically regenerated selectors/scalars plus captured profile skills/MCP/hooks and Bridge | OAuth credentials | fresh/restart/resume materialize the same private projection; source inventory/Studio composition and native extensions remain pending (`t-c9a086`, `t-115742`) | ~ |
