...
| Grok rows measured on 0.2.112/0.2.118, host runs 1.0.0 | Re-measure the remaining Grok cells against 1.0.0. Model preflight was re-measured and fixed in `t-5dcf47`: 1.0.0 prints a catalog in both auth states, so the banner is now checked fail-safe (§3.8). |
