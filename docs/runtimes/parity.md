...
| Pi | safe global JSON settings seeded once, executable resources removed; ambient `trust.json` currently crosses the boundary | exact declared resource snapshots | private agent/session homes; native trust is canonical-path/nearest-ancestor based | private auth snapshot; provider authority remains external | private settings/trust become runtime-owned after seed, so stale/external grants survive lifecycle; exact-trust follow-up `t-20c856` required (audit `t-68ee7a`) | ~ |
