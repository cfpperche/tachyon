...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  ...
  // 516 — a sincronização do lançador de ferramenta saiu com o provisionamento de binário. O Tachyon
  // não baixa mais binário: um plugin declara em `requires` a ferramenta externa de que precisa, a tela
  // detecta se ela está no PATH, e o operador a instala.


  /** 516 — a reconciliação do harness de git hook saiu com o sistema antigo: git hooks ficaram de
   *  fora da v1 e voltam como um sistema próprio. Não há dispatcher gerado a trazer para frente. */
