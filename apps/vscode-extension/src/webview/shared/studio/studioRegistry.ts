 ...
 * for its much larger profile domain-message surface), D2 (task), D3 (pin) each add
...
export const STUDIO_REGISTRY: Record<StudioId, StudioRegistryEntry> = {
  ...
  agent: {
    ...
    // the canonical-profile readers); the RUNTIME object (WorkspaceShellHandle) already implements
