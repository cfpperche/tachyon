...
export async function readCanonicalSoulBytes(workspaceRoot: string, name: string): Promise<Buffer | undefined> {
  ...
  try {
    ...
    const code = typeof error === "object" && error !== null && "code" in error ? String((error as { code?: unknown }).code) : "";
    if (error instanceof SoulError && (error.code === "soul/missing" || error.code === "soul/final-symlink")) return undefined;
...
export async function findFoldedProfileCollision(
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      /* missing/unowned manifests are not collisions */
    }
  }
  return undefined;
}
...
export async function inspectSoulProfile(
  ...
  try {
    ...
    } else if (!(error instanceof SoulError && error.code === "soul/missing")) {
      /* keep missing default for absent trees */
    }
  }
...
/** Overwrite only the private manifest (active↔retained) with create-only staging + replace via unlink+link. */
