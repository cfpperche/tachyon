...
import type { AgentStudioPatch, SoulProfileStatusMessage } from "./domain";
...
export const adoptSoulProfileMessage = (agent: string, expectedDigest?: string) =>
  envelope({ type: "adoptSoulProfile" as const, agent, ...(expectedDigest ? { expectedDigest } : {}) });
...
export const soulProfileStatusMessage = (status: SoulProfileStatusMessage) =>
  envelope({ type: "soulProfileStatus" as const, status });
