...
type Selection = { summary: string; tag?: string; selectorHint?: string; text?: string; screenshotUri?: string };
...
export function App() {
  ...
  return <main class="dm-app">
    ...
    {selection && <aside class="dm-selection" aria-label="Attached selection"><div class="dm-selection-title"><span>Attached selection</span><button class="icon" aria-label="Clear selection" onClick={() => post({ type: "designMode.clearSelection" })}>×</button></div>{selection.screenshotUri && <img class="dm-selection-shot" src={selection.screenshotUri} alt={selection.summary} />}<code>{selection.summary}</code>{selection.text && <p>{selection.text}</p>}</aside>}
