...
const post = (message: EngineAction) =>
  vscode ? vscode.postMessage(message) : window.postMessage(message, "*");

function Root() {
  const [model, setModel] = useState<CockpitModel>();
  const strings =
    (window as unknown as { __TACHYON_STRINGS__?: CockpitStrings }).__TACHYON_STRINGS__ ?? defaultStrings;
  useEffect(() => {
    const on = (e: MessageEvent) => {
      if (e.data?.type === ENGINE_MODEL) setModel(e.data.model);
    };
    window.addEventListener("message", on);
    post(readyMessage());
    const timer = setInterval(() => post(pollEngineAction()), 3000);
    return () => {
      clearInterval(timer);
