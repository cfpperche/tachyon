...
export class CommandStudioPanelManager {
  ...
  openExisting(ws: Workspace, commandName: string): void {
  ...
  captureSnapshot(ws: Workspace, entityId?: string): StudioRestoreSnapshot<string, CommandStudioPatch> | undefined {
    return this.workspaces.get(ws.wsHash)?.captureSnapshot(ws.wsHash, entityId);
  }

  restoreFromSnapshot(ws: Workspace, snapshot: StudioRestoreSnapshot<string, CommandStudioPatch>): void {
  ...
  private baseFor(ws: Workspace): StudioPanelManagerBase<CommandStudioEntity, CommandStudioFields, CommandStudioPatch, CommandStudioReferenceData> {
