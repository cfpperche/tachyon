 ...
 *
 * Extracted out of the vscode-bound `Workspace.ts` (which CI cannot import — see memory
 * `feedback_logic_in_vscode_layer_escapes_ci`) so the composition is unit-tested. The whole point of the
 * extraction is the EQUIVALENCE LOCK: a node with a `task` and no run input / no upstream summaries must
 * produce EXACTLY today's string (`${task}\n\n${GUIDANCE}`), so the 230 engine stays byte-identical.
 *
