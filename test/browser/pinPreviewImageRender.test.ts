...
describe("Pin Preview renders inline doc images (t-321e9d)", () => {
  ...
  it("renders a real <img> in the body instead of the literal '[Image]' placeholder", async () => {
    ...
    await loadPinPreview(page, server.origin, FIXTURE_VM);

    const bodyText = await page.$eval(".body", (el) => el.textContent ?? "");
    expect(bodyText).not.toContain("[Image]");

    const img = await page.$eval(".body img", (el) => (el as HTMLImageElement).src);
    ...
    const visualImg = await page.$eval(".visual img", (el) => (el as HTMLImageElement).src);
