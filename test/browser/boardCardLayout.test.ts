...
describe("SDD 419 — Board card metadata layout", () => {
  ...
  it("keeps author/badges and id/assignee-priority in independent opposite regions", async () => {
    ...
    const surface = await loadBoard(page, server.origin, { width: 1440, height: 900 });

    const result = await surface.$eval('[data-card-id="t-82f870"]', (card) => {
  ...
  it("keeps 300px lanes and delegates narrow overflow to the board scroller", async () => {
    ...
    const surface = await loadBoard(page, server.origin, { width: 900, height: 900 });

    const layout = await surface.$eval(".board", (board) => ({
