  ...
  it("offers review and propose without turning the land block into three competing buttons", async () => {
    ...
    for (const w of WIDTHS) {
      ...
      if (geom.ok) {
        ...
        for (const compare of geom.compareTexts) {
          expect(compare, `the base is named at ${w.px}px`).toContain("main");
          expect(compare, `the head is named at ${w.px}px`).toContain(HEAD.slice(0, 12));
        }
      }
