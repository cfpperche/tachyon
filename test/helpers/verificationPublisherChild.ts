...
const [workspace, recordFile, readyFile, startFile, resultFile] = process.argv.slice(2);
if (!workspace || !recordFile || !readyFile || !startFile || !resultFile) throw new Error("publisher child arguments are required");
...
try {
  ...
  const publishedPath = writeVerificationRecord(workspace, record);
