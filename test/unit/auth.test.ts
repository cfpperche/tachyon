...
describe("Bridge auth enforcement (live HTTP)", () => {
  ...
  it("rejects missing/wrong bearer with 401 and accepts the right one end-to-end", async () => {
    ...
    try {
      ...
      expect(tools.length).toBe(75); // SDD 443 adds continue_task; spec 444 adds worktree_hygiene; SDD 478 M9 adds spawn_terminal; t-e74631 adds reconcile_worktree_hygiene.
      await client.close();
    } finally {
  ...
  it("no token configured (settings.auth: false) keeps the Bridge open", async () => {
    ...
    try {
      ...
      expect((await client.listTools()).tools.length).toBe(75); // SDD 443 adds continue_task; spec 444 adds worktree_hygiene; SDD 478 M9 adds spawn_terminal; t-e74631 adds reconcile_worktree_hygiene.
      await client.close();
    } finally {
      await bridge.dispose();
    }
