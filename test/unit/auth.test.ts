...
describe("Bridge auth enforcement (live HTTP)", () => {
  ...
  it("rejects missing/wrong bearer with 401 and accepts the right one end-to-end", async () => {
    ...
    try {
      ...
      expect(tools.length).toBe(71); // t-e88c8a stage 1 removed the nine Delivery tools (78 → 69); t-f638bd added reconcile_task (69 → 70); t-0bebf6 added acknowledge_agent (70 → 71); bridge.test.ts holds the by-name inventory.
      await client.close();
    } finally {
  ...
  it("no token configured (settings.auth: false) keeps the Bridge open", async () => {
    ...
    try {
      ...
      expect((await client.listTools()).tools.length).toBe(71); // t-e88c8a stage 1 removed the nine Delivery tools (78 → 69); t-f638bd added reconcile_task (69 → 70); t-0bebf6 added acknowledge_agent (70 → 71); bridge.test.ts holds the by-name inventory.
      await client.close();
    } finally {
      await bridge.dispose();
    }
