...
describe("Bridge auth enforcement (live HTTP)", () => {
  ...
  it("rejects missing/wrong bearer with 401 and accepts the right one end-to-end", async () => {
    ...
    try {
      ...
      expect(tools.length).toBe(70); // spec 392 adds 6 managed-worktree tools; t-608f2e adds projection reconcile; t-ae89d1 adds cancel_human_approval.
      await client.close();
    } finally {
  ...
  it("no token configured (settings.auth: false) keeps the Bridge open", async () => {
    ...
    try {
      ...
      expect((await client.listTools()).tools.length).toBe(70); // spec 392 adds 6 managed-worktree tools; t-608f2e adds projection reconcile; t-ae89d1 adds cancel_human_approval.
      await client.close();
    } finally {
      await bridge.dispose();
    }
