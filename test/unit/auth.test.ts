...
describe("Bridge auth enforcement (live HTTP)", () => {
  ...
  it("rejects missing/wrong bearer with 401 and accepts the right one end-to-end", async () => {
    ...
    try {
      ...
      expect(tools.length).toBe(51); // spec 362 — verify_task added; spec t-7d8bdf added 3 approval tools; t-1339a8 added flag_for_human/clear_human_flag
      await client.close();
    } finally {
  ...
  it("no token configured (settings.auth: false) keeps the Bridge open", async () => {
    ...
    try {
      ...
      expect((await client.listTools()).tools.length).toBe(51); // spec 362 — verify_task added; spec t-7d8bdf added 3 approval tools; t-1339a8 added flag_for_human/clear_human_flag
      await client.close();
    } finally {
      await bridge.dispose();
    }
