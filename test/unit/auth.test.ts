...
describe("Bridge auth enforcement (live HTTP)", () => {
  ...
  it("rejects missing/wrong bearer with 401 and accepts the right one end-to-end", async () => {
    ...
    try {
      ...
      expect(tools.length).toBe(27); // spec 255 — get_pin adds rich-pin detail read
      await client.close();
    } finally {
  ...
  it("no token configured (settings.auth: false) keeps the Bridge open", async () => {
    ...
    try {
      ...
      expect((await client.listTools()).tools.length).toBe(27); // spec 255 — get_pin adds rich-pin detail read
      await client.close();
    } finally {
      await bridge.dispose();
    }
