...
function makeViewOnlyPlugin(opts: { name?: string; version?: string; entry?: string; actions?: string[]; title?: string } = {}): string {
  const dir = tmp("tachyon-plugin-");
  const entry = opts.entry ?? "ui/index.html";
  fs.mkdirSync(path.join(dir, path.dirname(entry)), { recursive: true });
  fs.writeFileSync(path.join(dir, entry), "<!doctype html><meta charset=\"utf-8\"><script>postMessage('ready','*')</script>\n");
  fs.writeFileSync(path.join(dir, "tachyon-plugin.json"), JSON.stringify({
    name: opts.name ?? "mundinho",
    version: opts.version ?? "1.0.0",
    description: "view only",
    views: [{ id: "agents", title: opts.title ?? "Agents", surface: "editor", entry, fleet: "summary", actions: opts.actions ?? ["focusAgent"] }],
  }));
  return dir;
}
...
describe("loadPlugin — skills discovery (spec 251)", () => {
  it("discovers skills from the neutral skills/ payload, sorted by name, alongside hooks", async () => {
  ...
  it("loads a SKILLS-ONLY plugin (no hooks blocks)", async () => {
  ...
  it("rejects a plugin with NO capability (no hooks, no skills)", async () => {
  ...
  it("ignores a subdir under skills/ that has no SKILL.md", async () => {
  ...
  it("propagates a malformed SKILL.md as a fail-closed error", async () => {
    const dir = makeSkillsOnlyPlugin();
  ...
  it("rejects a skills/ that is a symlink (no enumeration escape outside the plugin)", async () => {
    const dir = makeSkillsOnlyPlugin();
  ...
  it("rejects a symlinked skill entry under skills/", async () => {
    const dir = makeSkillsOnlyPlugin();
  ...
  it("caps the immediate skills/ fanout (hostile empty-dir flood)", async () => {
    const dir = makeSkillsOnlyPlugin();
  ...
  it("caps the fanout even with regular FILES (not just dirs/symlinks)", async () => {
    const dir = makeSkillsOnlyPlugin();
    const root = path.join(dir, "skills");
...
describe("loadPlugin — MCP discovery (spec 254 Step 1)", () => {
  ...
  it("loads an MCP-only plugin (no hooks/skills) — MCP counts toward the ≥1-capability rule", async () => {
  ...
  it("fails closed on an invalid mcp.json", async () => {
    const dir = makePlugin({ runtimes: ["claude"] });
  ...
  it("rejects a symlinked mcp.json (no escaping the plugin boundary)", async () => {
    const dir = makePlugin({ runtimes: ["claude"] });
  ...
  it("loads a mixed hooks + skills + MCP plugin", async () => {
    const dir = makePlugin({ runtimes: ["claude"] });
    addSkill(dir, "my-skill");
  ...
  it("absent mcp.json → no MCP (not an error)", async () => {
    const dir = makePlugin({ runtimes: ["claude"] });
    const { plugin, errors } = loadPlugin(dir);
    expect(errors).toEqual([]);
...
describe("planMcpTargets + per-runtime renderers (spec 254 Step 2)", () => {
  ...
  it("runtimeSupportsMcp is true for both v1 runtimes", async () => {
    expect(runtimeSupportsMcp("claude")).toBe(true);
    expect(runtimeSupportsMcp("codex")).toBe(true);
  });

  it("plans each server × each present declared runtime, in runtime then server order; skips absent runtimes", async () => {
    const dir = makePlugin({ runtimes: ["claude", "codex"] });
  ...
  it("no-MCP plugin plans nothing", async () => {
    const dir = makePlugin({ runtimes: ["claude"] });
    const { plugin } = loadPlugin(dir);
    expect(planMcpTargets(plugin!, new Set(["claude"] as const))).toEqual([]);
  });

  it("renders a claude stdio + http entry (verbatim, empties dropped)", async () => {
  ...
  it("renders a codex http TOML block — Bearer→bearer_token_env_var, other header→env_http_headers", async () => {
    expect(renderCodexMcpBlock(oneServer(HTTP))).toBe(
      `[mcp_servers.api]\nurl = "https://api.test/v1"\nbearer_token_env_var = "API_TOKEN"\nenv_http_headers = { "X-Env" = "ENVNAME" }\n`,
    );
  });

  it("TOML-escapes a hazardous literal arg + a unicode arg (no injection)", async () => {
    const s = oneServer({ name: "x", transport: "stdio", command: "node", args: ['a"b\\c', "café→x"] });
    expect(renderCodexMcpBlock(s)).toBe(`[mcp_servers.x]\ncommand = "node"\nargs = ["a\\"b\\\\c", "café→x"]\n`);
  });

  it("renders a hyphenated server name as a safe bare key + multiple env_vars", async () => {
  ...
  it("renders multiple non-auth headers into one env_http_headers table", async () => {
...
describe("MCP install / remove I/O (spec 254 Step 4)", () => {
  ...
  it("collision: undecided → fail-closed; Keep leaves the user's server; Replace overwrites", async () => {
    ...
    expect((await applyInstall(plugin!, preview, ws, detectRuntimes(ws), { mcpConfirmed: true })).errors[0]).toMatch(/collides/);
  ...
  it("[security] OQ5: applyInstall refuses MCP without the explicit confirmation (engine fail-closed, not just UI)", async () => {
    const ws = makeWorkspace(["claude"]);
    const { plugin } = loadPlugin(mcpPlugin([STDIO], ["claude"]));
    const preview = previewInstall(plugin!, ws, detectRuntimes(ws));
    // no mcpConfirmed → refuse
    expect((await applyInstall(plugin!, preview, ws, detectRuntimes(ws))).errors[0]).toMatch(/MCP servers require/);
    // mcpConfirmed:true → installs
    expect((await applyInstall(plugin!, preview, ws, detectRuntimes(ws), { mcpConfirmed: true })).installed).toBe(true);
  });

  it("[security] lost-update: .mcp.json changed between preview and apply → refuse", async () => {
    const ws = makeWorkspace(["claude"]);
    const { plugin } = loadPlugin(mcpPlugin([STDIO], ["claude"]));
    const preview = previewInstall(plugin!, ws, detectRuntimes(ws));
    fs.writeFileSync(MCPJSON(ws), JSON.stringify({ mcpServers: { other: { command: "x" } } })); // user edits after preview
    expect((await applyInstall(plugin!, preview, ws, detectRuntimes(ws), { mcpConfirmed: true })).errors[0]).toMatch(/changed since preview/);
  });

  it("[security] a broken .mcp.json fails closed at preview (never treated as 'server absent')", async () => {
    const ws = makeWorkspace(["claude"]);
  ...
  it("[security] a forged prior target can't claim a user's server: content mismatch ⇒ still a collision", async () => {
    const ws = makeWorkspace(["claude"]);
    fs.writeFileSync(MCPJSON(ws), JSON.stringify({ mcpServers: { db: { command: "USER-OWN" } } }));
  ...
  it("codex content-aware remove preserves model= + other tables; edited block → orphan", async () => {
...
describe("skill install / remove I/O (spec 251 Step 3)", () => {
  ...
  it("materializes a skill into each present runtime's skills dir + records skill-dir targets", async () => {
    ...
    const ws = makeWorkspace(["claude", "codex"]);
    const res = await installWith(dir, ws);
  ...
  it("a hooks+skills plugin installs both", async () => {
    const dir = makePlugin({ runtimes: ["claude"] });
    addSkill(dir, "helper");
    const ws = makeWorkspace(["claude"]);
    expect((await installWith(dir, ws)).installed).toBe(true);
  ...
  it("refuses (fail-closed) a colliding skill with no Keep/Replace decision", async () => {
    ...
    const res = await installWith(dir, ws); // no decision
  ...
  it("Keep leaves the user's skill untouched and does not record it", async () => {
    const dir = makeSkillsOnlyPlugin("sk", ["claude"]);
    addSkill(dir, "kept");
    ...
    const res = await installWith(dir, ws, { ".claude/skills/kept": "keep" });
    expect(res.installed).toBe(true);
  ...
  it("Replace overwrites the user's skill (consented) and records it", async () => {
    ...
    const res = await installWith(dir, ws, { ".claude/skills/dup": "replace" });
    expect(res.installed).toBe(true);
    expect(fs.readFileSync(SKILL(ws, ".claude", "dup"), "utf8")).toMatch(/name: dup/); // overwritten with the plugin's
  });

  it("remove deletes the materialized skill-dirs", async () => {
    const dir = makeSkillsOnlyPlugin("sk", ["claude"]);
    addSkill(dir, "gone");
    const ws = makeWorkspace(["claude"]);
  ...
  it("remove fail-closes on a corrupted skill-dir target (never deletes an arbitrary path)", async () => {
    const ws = makeWorkspace(["claude"]);
    ...
    expect((await applyRemove("sk", ws)).removed).toBe(false);
    expect(fs.existsSync(path.join(ws, "package.json"))).toBe(true); // NOT deleted
  });

  it("install fail-closes on a pre-existing corrupted skill-dir target (won't stale-delete an arbitrary path)", async () => {
    const ws = makeWorkspace(["claude"]);
    const dir = makeSkillsOnlyPlugin("sk", ["claude"]); addSkill(dir, "s1");
    await installWith(dir, ws);
    const lock = readJson(LOCK(ws));
    ...
    const res = await installWith(dir, ws); // a re-install
    expect(res.installed).toBe(false);
...
describe("planSkillTargets (spec 251 Step 2)", () => {
  it("plans claude → .claude/skills and codex → .agents/skills for each present runtime", async () => {
    const dir = makeSkillsOnlyPlugin("sk", ["claude", "codex"]);
    addSkill(dir, "alpha");
    addSkill(dir, "beta");
    const { plugin } = loadPlugin(dir);
  ...
  it("skips a declared runtime that is ABSENT from the workspace", async () => {
    const dir = makeSkillsOnlyPlugin("sk", ["claude", "codex"]);
    addSkill(dir, "alpha");
    const { plugin } = loadPlugin(dir);
...
describe("loadPlugin", () => {
  it("reads + validates a plugin's manifest and claude hooks", async () => {
  ...
  it("fail-closes on a missing manifest / missing hooks", async () => {
...
describe("detectRuntimes", () => {
  it("detects claude by its config dir; nothing in a bare workspace", async () => {
...
describe("previewInstall (the security surface)", () => {
  it("surfaces the wired commands + diff + a fingerprint without writing anything", async () => {
  ...
  it("skips a declared runtime absent from the workspace", async () => {
    const ws = makeWorkspace();
  ...
  it("fail-closes on an invalid existing settings.json (never treats it as empty)", async () => {
    const ws = makeWorkspace();
  ...
  it("fail-closes on a corrupt lockfile", async () => {
    const ws = makeWorkspace();
  ...
  it("fail-closes when the lockfile path is present-but-unreadable (EISDIR, not absent)", async () => {
    const ws = makeWorkspace();
  ...
  it("rejects a payload containing a symlink", async () => {
...
describe("git-hook target — load + preview + fingerprint (spec 264)", () => {
  ...
  it("loadPlugin discovers a script git-hook and an argv git-hook; a git-hook-only plugin is a valid capability", async () => {
  ...
  it("previewInstall errors when the plugin declares git-hooks but the workspace is not a git repo / has worktreeConfig", async () => {
    const ws = tmp("tachyon-ws-");
    const { plugin } = loadPlugin(makeGitHookPlugin());
  ...
  it("the fingerprint binds the git state: hooksPath and prior-hook drift change it", async () => {
    const ws = tmp("tachyon-ws-");
    const { plugin } = loadPlugin(makeGitHookPlugin());
...
describe("previewInstall — declared-runtime targeting (spec 263)", () => {
  ...
  it("a FRESH workspace (no .claude/.codex) still plans BOTH declared runtimes — the target set, not what exists", async () => {
  ...
  it("deselecting a runtime drops it to skipped and plans only the kept one", async () => {
    const ws = tmp("tachyon-ws-");
    const { plugin } = loadPlugin(makePlugin({ runtimes: ["claude", "codex"] }));
  ...
  it("GOLDEN: a both-present workspace produces the unchanged install plan (present-path regression, scenario 63)", async () => {
  ...
  it("binds the runtime SELECTION into the fingerprint even for a no-artifact runtime", async () => {
    const ws = tmp("tachyon-ws-");
  ...
  it("atomicWrite removes its temp file when the rename fails — no orphan temp (spec 263 task 5)", async () => {
...
describe("uninstall — created-ancestor cleanup (spec 263 task 6)", () => {
  const installBoth = async (ws: string) => {
    const pdir = makePlugin({ runtimes: ["claude", "codex"] });
    addSkill(pdir, "deploy");
    const { plugin } = loadPlugin(pdir);
    const target = new Set(["claude", "codex"] as const);
  ...
  it("rmdir's exactly the runtime dirs the install created, deepest-first", async () => {
    const ws = tmp("tachyon-ws-"); // genuinely fresh
    expect((await installBoth(ws)).installed).toBe(true);
  ...
  it("never removes a runtime dir that pre-existed the install", async () => {
  ...
  it("leaves a created dir the user later filled with unrelated content (non-empty → safe no-op)", async () => {
    const ws = tmp("tachyon-ws-");
    await installBoth(ws);
  ...
  it("an old lock with no createdAncestors removes cleanly (ancestor cleanup is a no-op)", async () => {
    const ws = makeWorkspace(["claude"]);
    await install(makePlugin(), ws);
  ...
  it("a PARTIAL install (a skill write fails mid-activation) still records a complete removal manifest incl. createdAncestors", async () => {
    const ws = tmp("tachyon-ws-");
...
describe("install → use → remove (end-to-end on a real temp workspace)", () => {
  ...
  it("is idempotent — re-installing does not duplicate the hook", async () => {
    const ws = makeWorkspace();
    await install(makePlugin(), ws);
    await install(makePlugin(), ws);
  ...
  it("surfaces an orphan when the user edited the plugin's hook before removing", async () => {
    const ws = makeWorkspace();
    await install(makePlugin(), ws);
    ...
    expect((await applyRemove("sdd", ws)).orphans).toBe(1);
    expect(readJson(SETTINGS(ws)).hooks.PreToolUse).toHaveLength(1); // edited group left, never deleted
  });

  it("binds a remove to its consent fingerprint (TOCTOU) — refuses a stale one, honors a matching one", async () => {
    const ws = makeWorkspace();
    await install(makePlugin(), ws);
    const fp = previewRemove("sdd", ws).fingerprint;
    expect(fp).not.toBe("");
    // a fingerprint that no longer matches current state → refuse, nothing removed
    const stale = await applyRemove("sdd", ws, { expectedFingerprint: "STALE" });
    ...
    expect((await applyRemove("sdd", ws, { expectedFingerprint: fp })).removed).toBe(true);
    expect(fs.existsSync(LOCK(ws))).toBe(false);
  });
});

describe("safety + fail-closed", () => {
  it("apply refuses a stale preview (settings changed since consent)", async () => {
    const ws = makeWorkspace();
    const { plugin } = loadPlugin(makePlugin());
    const preview = previewInstall(plugin!, ws, detectRuntimes(ws));
    fs.writeFileSync(SETTINGS(ws), JSON.stringify({ hooks: { PreToolUse: [{ hooks: [{ type: "command", command: "snuck-in" }] }] } }));
    const res = await applyInstall(plugin!, preview, ws, detectRuntimes(ws));
    expect(res.installed).toBe(false);
    expect(res.errors[0]).toMatch(/changed since preview/);
  });

  it("apply refuses when there is nothing to install (no runtime present)", async () => {
  ...
  it("remove fail-closes on a hand-corrupted lockfile removal", async () => {
    ...
    expect((await applyRemove("sdd", ws)).removed).toBe(false);
  });

  it("remove reports not-installed for an unknown plugin", async () => {
    const ws = makeWorkspace();
    expect(previewRemove("ghost", ws).found).toBe(false);
    expect((await applyRemove("ghost", ws)).errors[0]).toMatch(/not installed/);
  });
});
...
describe("update (3-way: baseline vs current vs new)", () => {
  ...
  it("auto-updates when the user hasn't edited the plugin's hooks", async () => {
    const ws = makeWorkspace();
    ...
    const res = await applyUpdate(v2, ws);
  ...
  it("is a no-op when already up to date", async () => {
    const ws = makeWorkspace();
    await install(makePlugin(), ws);
    const res = await applyUpdate(loadPlugin(makePlugin()).plugin!, ws);
    expect(res).toMatchObject({ updated: false, upToDate: true });
  });

  it("refuses an update whose new version drops a runtime the install uses (spec 263 task 7)", async () => {
    const ws = makeWorkspace(["claude", "codex"]);
    ...
    expect((await applyUpdate(v2, ws)).updated).toBe(false);
  ...
  it("an update materializes only the consented installed set, never a newly-added runtime (spec 263 task 7)", async () => {
  ...
  it("refuses to update a plugin that isn't installed", async () => {
    const ws = makeWorkspace();
    expect((await applyUpdate(loadPlugin(makePlugin()).plugin!, ws)).errors[0]).toMatch(/not installed/);
  });

  it("refuses without force when the user edited the plugin's hook (3-way conflict)", async () => {
    const ws = makeWorkspace();
    await install(makePlugin({ command: V1 }), ws);
    const s = readJson(SETTINGS(ws));
  ...
  it("with force, updates despite the edit, keeping the edited group as a conservative orphan", async () => {
    const ws = makeWorkspace();
    await install(makePlugin({ command: V1 }), ws);
    const s = readJson(SETTINGS(ws));
    s.hooks.PreToolUse[0].hooks[0].command = "MY-EDIT";
    fs.writeFileSync(SETTINGS(ws), JSON.stringify(s));
    const v2 = loadPlugin(makePlugin({ version: "2.0.0", command: V2 })).plugin!;
    const res = await applyUpdate(v2, ws, { force: true });
    expect(res.updated).toBe(true);
    const cmds = cmdOf(ws);
  ...
  it("refuses when the user already added a hook equal to the new version (would-duplicate collision)", async () => {
    const ws = makeWorkspace();
    await install(makePlugin({ command: V1 }), ws);
  ...
  it("refuses a downgrade without force", async () => {
    const ws = makeWorkspace();
    await install(makePlugin({ version: "2.0.0", command: V2 }), ws);
    const v1 = loadPlugin(makePlugin({ version: "1.0.0", command: V1 })).plugin!;
    const preview = await previewUpdate(v1, ws);
    expect(preview.isDowngrade).toBe(true);
    expect((await applyUpdate(v1, ws)).errors[0]).toMatch(/lower than|downgrade/);
    expect((await applyUpdate(v1, ws, { force: true })).updated).toBe(true); // force allows it
  });

  it("binds an update to its consent fingerprint (TOCTOU) — refuses a stale one, applies a matching one", async () => {
    const ws = makeWorkspace();
    await install(makePlugin({ command: V1 }), ws);
    const v2 = loadPlugin(makePlugin({ version: "2.0.0", command: V2 })).plugin!;
    const fp = (await previewUpdate(v2, ws)).install!.fingerprint;
    // a fingerprint that doesn't match the fresh plan → refuse, no write
    const stale = await applyUpdate(v2, ws, { expectedFingerprint: "STALE" });
    expect(stale.updated).toBe(false);
    expect(stale.errors[0]).toMatch(/changed since preview/);
    expect(cmdOf(ws)[0]).toContain("v1.sh"); // untouched
    // the consented fingerprint → applies
    expect((await applyUpdate(v2, ws, { expectedFingerprint: fp })).updated).toBe(true);
    expect(cmdOf(ws)[0]).toContain("v2.sh");
  });

  it("multi-runtime: a conflict in only one runtime is surfaced for that runtime", async () => {
    const ws = makeWorkspace(["claude", "codex"]);
    await install(makePlugin({ runtimes: ["claude", "codex"], command: V1 }), ws);
  ...
  it("wires the SAME plugin into BOTH claude and codex (the multi-runtime thesis)", async () => {
    const ws = makeWorkspace(["claude", "codex"]);
    const res = await install(makePlugin({ runtimes: ["claude", "codex"] }), ws);
    expect(res.installed).toBe(true);
    expect(res.runtimes.sort()).toEqual(["claude", "codex"]);
  ...
  it("a claude+codex plugin in a claude-only workspace wires claude, skips codex", async () => {
    const ws = makeWorkspace(["claude"]);
    const res = await install(makePlugin({ runtimes: ["claude", "codex"] }), ws);
  ...
  it("install aborts if the plugin payload gains a symlink after preview (TOCTOU)", async () => {
    const ws = makeWorkspace();
    const pluginDir = makePlugin();
    const { plugin } = loadPlugin(pluginDir);
    const preview = previewInstall(plugin!, ws, detectRuntimes(ws));
