...
describe("Human Inbox view model — one list, one count", () => {
  it("counts both kinds together and each kind on its own, from the rows themselves", () => {
    ...
    expect(vm.counts).toEqual({ total: 3, approvals: 2, savedAgentProposals: 0, savedAgentRemovals: 0, scheduleProposals: 0, validations: 1, stale: 0 });
  });
  ...
  it("an empty inbox is a zeroed count, not an absent one", () => {
    expect(build([], []).counts).toEqual({ total: 0, approvals: 0, savedAgentProposals: 0, savedAgentRemovals: 0, scheduleProposals: 0, validations: 0, stale: 0 });
  });
});
