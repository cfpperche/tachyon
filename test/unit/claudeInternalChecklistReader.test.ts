...
import {
  CLAUDE_INTERNAL_CHECKLIST_TOOLS,
  CLAUDE_SUBAGENT_TOOLS,
  claudeInternalChecklistToolsPresent,
  readClaudeInternalChecklist,
} from "@tachyon/engine/runtime/claudeInternalChecklistReader.js";
 ...
 *
 * These tests import `readClaudeInternalChecklist` — the host door. A helper-only parser that
...
describe("t-96c1b3 — measured Claude plan-tool inventory", () => {
  it("flag-off init.tools has the subagent family and no TaskCreate family", () => {
    ...
    for (const name of CLAUDE_INTERNAL_CHECKLIST_TOOLS) {
      expect(FLAG_OFF_TOOLS.tools).not.toContain(name);
    }
    expect(claudeInternalChecklistToolsPresent(FLAG_OFF_TOOLS.tools)).toBe(false);
  });
  ...
  it("flag-on init.tools has the TaskCreate family, still distinct from the subagent family", () => {
    ...
    expect(FLAG_ON_TOOLS.tools).toEqual(expect.arrayContaining([...CLAUDE_INTERNAL_CHECKLIST_TOOLS]));
    expect(FLAG_ON_TOOLS.tools).toEqual(expect.arrayContaining([...CLAUDE_SUBAGENT_TOOLS]));
    expect(claudeInternalChecklistToolsPresent(FLAG_ON_TOOLS.tools)).toBe(true);
  });

  it("does not treat Task / TaskOutput / TaskStop as the plan channel", () => {
    expect(claudeInternalChecklistToolsPresent(["Task", "TaskOutput", "TaskStop"])).toBe(false);
    expect(claudeInternalChecklistToolsPresent(["TaskCreate", "TaskGet", "TaskList", "TaskUpdate"])).toBe(true);
  });
});

describe("t-96c1b3 — readClaudeInternalChecklist (production door)", () => {
  it("projects the measured authenticated session with id and blockedBy preserved", () => {
    const plan = readClaudeInternalChecklist({
    ...
    expect(plan).toEqual({
      state: "snapshot",
      items: [
        { id: "1", text: "PLAN_B_4VJ8P2 root", status: "pending" },
        { id: "2", text: "PLAN_B_4VJ8P2 child", status: "pending", blockedBy: ["1"] },
      ],
    });
  });
  ...
  it("is mute when the session store is absent, and empty when the channel spoke with no items", () => {
    const home = tempHome();
    expect(readClaudeInternalChecklist({ configHome: home, sessionId: MEASURED_SESSION })).toEqual({
      state: "mute",
    });
    fs.mkdirSync(path.join(home, "tasks", "spoken-empty"), { recursive: true });
    fs.writeFileSync(path.join(home, "tasks", "spoken-empty", ".lock"), "");
    expect(readClaudeInternalChecklist({ configHome: home, sessionId: "spoken-empty" })).toEqual({
      state: "snapshot",
      items: [],
    });
  });
  ...
  it("fails if the reader does not open the store (red proof)", () => {
    ...
    expect(readClaudeInternalChecklist({ configHome: home, sessionId })).toEqual({
      state: "snapshot",
      items: [{ id: "9", text: "CANARY_STORE_READ", status: "completed", blockedBy: ["8"] }],
    });
  });
  ...
  it("does not invent an id when the store row has none", () => {
    ...
    expect(readClaudeInternalChecklist({ configHome: home, sessionId })).toEqual({
      state: "snapshot",
      items: [{ text: "no identity", status: "in-progress" }],
    });
  });

  it("this suite calls the production reader, not a test-local stand-in", () => {
    const source = fs.readFileSync(path.resolve("test/unit/claudeInternalChecklistReader.test.ts"), "utf8");
    expect(source).toMatch(/from "@tachyon\/engine\/runtime\/claudeInternalChecklistReader\.js"/);
    expect(source).toMatch(/readClaudeInternalChecklist\(/);
    expect(source).not.toMatch(/function readClaudeInternalChecklist\(/);
  });
});
