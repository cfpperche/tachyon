...
describe("CONTROL_SECTION_NAV (t-6e2952)", () => {
  ...
  it("marks exactly the standalone-app tiles, and only those (SDD 485)", () => {
    ...
    // a migration changes a tile's destination and never its position (SDD 485 D3 is the fourth to prove it).
    expect(standalone).toEqual(["mission", "runtime", "tmux", "plugins"]);
