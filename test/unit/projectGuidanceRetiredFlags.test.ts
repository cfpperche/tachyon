...
const DEV_HOST_COMMANDS = ["scripts/dev-host/cli.sh", "dev-host/pointer.mjs"];
 ...
 * across lines — the shipped guidance's own `scripts/dev-host/cli.sh …` example does).
 *
 ...
 * removal is invisible here while `scripts/dev-host/cli.sh point --owner me` is not. Whitespace
...
describe("the transported project guidance never instructs a retired dev-host flag (t-3e5072)", () => {
  ...
  it("keeps `--worktree` usable — the guard bans the retired set, not dev-host flags in general", () => {
    ...
    expect(retiredFlagInstructions("arm another checkout with `scripts/dev-host/cli.sh point --worktree /wt/x --fixture s`")).toEqual([]);
...
describe("an instruction is distinguished from a mention", () => {
  it("catches a retired flag inside a dev-host command — the regression this guards", () => {
    const drifted = "cd to YOUR worktree and arm it with `scripts/dev-host/cli.sh point --owner me --fixture sample`.";
    expect(retiredFlagInstructions(drifted)).toEqual([{ flag: "--owner", command: "scripts/dev-host/cli.sh point --owner me --fixture sample" }]);
  });

  it("catches one inside a fenced block too", () => {
    const drifted = "```sh\nscripts/dev-host/cli.sh point-clear --all\n```";
  ...
  it("ignores a retired flag in a command that is not a dev-host one", () => {
    ...
    expect(retiredFlagInstructions("release it with `node scripts/dogfood/run.mjs lane -- release --owner me`")).toEqual([]);
  });

  it("does not fire on a flag that merely starts with a retired one", () => {
    expect(retiredFlagInstructions("`scripts/dev-host/cli.sh point --allow-dirty`")).toEqual([]);
  });

  it("reports every retired flag a single drifted command carries", () => {
    const drifted = "`scripts/dev-host/cli.sh point --slot 2 --activate --require-owner`";
