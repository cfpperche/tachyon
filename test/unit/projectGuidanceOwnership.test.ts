...
describe(`${"project-guidance-ownership"}: project-guidance ownership boundary`, () => {
  ...
  describe("t-486f43: no working METHOD is born immune to project guidance", () => {
    ...
    const PRODUCT_FACT = [
      ...
      {
        why: "continuity durability — what survives, and that it dies with the workspace, are properties of the continuity store (t-a1ee7e corrected the overclaim the 2026-08-21 rm -rf disproved)",
      ...
      { why: "that the two records disagree, and that no reading of them settles it — the resolution METHOD was released to the project (t-a1ee7e)", line: /^ {2}- If BOTH name DIFFERENT BOARD work/ },
      { why: "the boundary declaration this whole block answers to — scoped by t-a1ee7e to mechanism, naming the project as the owner of method", line: /^Every statement above is a property of Tachyon/ },
