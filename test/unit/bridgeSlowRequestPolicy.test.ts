...
describe("BridgeSlowRequestToastPolicy", () => {
  it("keeps normal 10s slow completions metric-only", () => {
    ...
    expect(policy.decide({ slow: true, durationMs: 10_500, tool: "append_task_note", claimedIdentity: "codex" })).toBeUndefined();
  });

  it("does not toast known long-running tools", () => {
    const policy = new BridgeSlowRequestToastPolicy({ now: 1_000, extremeSlowMs: 60_000 });

    expect(policy.decide({ slow: true, durationMs: 974_000, tool: "wait_for_agent", claimedIdentity: "grok" })).toBeUndefined();
  });

  it("includes tool and caller context for extreme slow requests", () => {
    const policy = new BridgeSlowRequestToastPolicy({ now: 1_000, extremeSlowMs: 60_000 });

    expect(policy.decide({ slow: true, durationMs: 120_000, tool: "append_task_note", caller: { kind: "agent", name: "cxSlowBridge" } })?.message).toBe(
  ...
  it("rate-limits identical extreme slow toasts", () => {
    ...
    const info = { slow: true, durationMs: 120_000, tool: "append_task_note", caller: { kind: "agent" as const, name: "cxSlowBridge" } };
  ...
  it("falls back to claimed identity when the resolved caller is legacy", () => {
    expect(bridgeSlowRequestMessage({ slow: true, durationMs: 90_000, tool: "notify_agent", claimedIdentity: "codex", caller: { kind: "legacy" } })).toBe(
