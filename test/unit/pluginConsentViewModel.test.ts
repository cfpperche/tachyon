...
describe("buildRemoveConsent", () => {
  it("summarizes removed groups + orphans and uses the remove fingerprint as token (TOCTOU)", () => {
    const preview: RemovePreview = { found: true, orphans: 1, removedCount: 3, expectedCount: 4, fingerprint: "rm-fp-77", errors: [] };
  ...
  it("a not-found plugin is a confirm-disabling error", () => {
    const vm = buildRemoveConsent("ghost", "0.0.0", { found: false, orphans: 0, removedCount: 0, expectedCount: 0, fingerprint: "", errors: [] });
