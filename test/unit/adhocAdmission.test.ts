...
/**
 * SDD 478 M9 (`t-8f3f7d`) — the ad-hoc door stops inferring what it creates.
 *
 * Before this, `spawn_agent` took any command and `suggestKindForCommand` decided the outcome: a name
...
describe("t-8f3f7d — what the ad-hoc door admits", () => {
  it.each(SUPPORTED_ADHOC_AGENT_RUNTIME_NAMES)("admits %s, the plain form", (runtime) => {
    expect(admitAdhocAgentCommand(runtime)).toEqual({ ok: true, runtime });
  });
  ...
  it.each([
    ...
    expect(admitAdhocAgentCommand(cmd)).toEqual({ ok: true, runtime });
  });
});

describe("t-8f3f7d — what it refuses, and what the refusal says", () => {
  it.each([
    ["a bare shell", "sh"],
  ...
  it("refuses an empty command without pretending to have resolved one", () => {
    expect(admitAdhocAgentCommand("   ")).toEqual({
      ok: false,
  ...
  it("refuses the authoring catalog's unbacked names — a quick-add chip is not evidence", () => {
    ...
    const catalogOnly = KNOWN_AI_CLIS.filter((cli) => !isSupportedAdhocAgentRuntime(cli));
    expect(catalogOnly.length).toBeGreaterThan(0);
    for (const cli of catalogOnly) {
      expect(admitAdhocAgentCommand(cli).ok).toBe(false);
    }
  });
});
...
describe("t-8f3f7d — the declaration is evidence, and cannot drift from it", () => {
  ...
  it("the declared brief channel agrees with the code that delivers one", () => {
    for (const [runtime, support] of Object.entries(SUPPORTED_ADHOC_AGENT_RUNTIMES)) {
  ...
  it("stays a SEPARATE list from canonical attestation, in both directions", () => {
    ...
    // driven ad-hoc, but the reverse is deliberately false — collapsing the two would have deleted
    // OpenCode, Hermes, Gemini and Qwen as agents as a side effect of a migration.
    for (const attested of ATTESTED_RUNTIMES) expect(isSupportedAdhocAgentRuntime(attested)).toBe(true);
    const adhocOnly = SUPPORTED_ADHOC_AGENT_RUNTIME_NAMES.filter((runtime) => !isAttestedRuntime(runtime));
    expect(adhocOnly).toEqual(["opencode", "hermes", "gemini", "qwen"]);
    for (const runtime of SUPPORTED_ADHOC_AGENT_RUNTIME_NAMES) {
      expect(SUPPORTED_ADHOC_AGENT_RUNTIMES[runtime].canonicalProfile).toBe(isAttestedRuntime(runtime));
    }
  });
  ...
  it("declares nothing it cannot resume — antigravity and continue are absent on purpose", () => {
    ...
    for (const runtime of ["antigravity", "continue"]) {
      ...
      expect(isSupportedAdhocAgentRuntime(runtime)).toBe(false);
    }
  });
});
