...
import { workspaceCommandSuccessV1, workspaceBoardViewSuccessV1 } from "../../src/engine-service/protocol.js";
...
describe("Workspace Board target", () => {
  it("reads one remote board and routes every mutation through a fresh operation identity", async () => {
    ...
    const target = workspaceBoardTarget(fake);

    expect(target.declaredAgentNames()).toEqual(["codex"]);
    expect(await target.listBoardAgents()).toEqual([
    ...
    expect(fake.invocations.map((entry) => entry.operationId)).toEqual([
      expect.stringMatching(/^board:[0-9a-f-]{36}$/),
      expect.stringMatching(/^board:[0-9a-f-]{36}$/),
      expect.stringMatching(/^board:[0-9a-f-]{36}$/),
      expect.stringMatching(/^board:[0-9a-f-]{36}$/),
    ]);
  ...
  it("does not turn a typed engine refusal into apparent success", async () => {
    const identity = projectionIdentity("/tmp/board-target-error");
    const fake = new FakeWorkspaceClient({
      identity,
    ...
    await expect(workspaceBoardTarget(fake).updateTask("t-abc123", { status: "triaged" }))
  ...
  it("reports a truncated agent projection as unavailable instead of silently omitting live filters", async () => {
    const identity = projectionIdentity("/tmp/board-target-truncated");
    ...
    await expect(workspaceBoardTarget(fake).listBoardAgents()).rejects.toThrow(/truncated/);
  });
});
