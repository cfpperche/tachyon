...
/**
 * 516 — dois blocos saíram daqui: os portões "retidos" da projeção de hook de plugin.
 *
 * Eles comparavam o que `settings.agentHookProjection` classificava com o que a sessão de fato
 * recebeu, e existiam porque instalar um plugin mesclava hooks no projeto — um agente isolado
 * precisava de uma classificação nomeada para que o portão o alcançasse mesmo assim. No sistema novo
 * um hook é capacidade de runtime como uma skill: chega porque foi concedido, e vai para a home do
 * agente. Sem hook de plugin solto no projeto, não há retenção a reportar.
 */
