...
describe("loadProfileAwareConfig", () => {
  ...
  // t-09be02 / 516 — o bloco que ficava aqui testava a TERCEIRA resposta da checagem de entradas
  // ambientes do Grok: "projetado", quando o próprio Tachyon tinha escrito em `.grok/*` ao instalar um
  // plugin e o lockfile registrava que era nosso. O 516 tira a instalação de dentro do projeto, então
  // essa resposta não tem como acontecer e a checagem virou "existe? então bloqueia".
  //
  // A cobertura do que sobrou já está no caso "Redirecting GROK_HOME does not stop project discovery"
  // acima, que planta um `.grok/config.toml` e espera a recusa nominal. Reescrever os seis casos daqui
  // teria sido seis formas de perguntar a mesma coisa que aquele já pergunta.
