...
describe("t-86e59a — `resolvedBy` names a channel, never an actor", () => {
  ...
  it("no write door in src/ passes a `resolvedBy` literal — every one names a declared channel constant", () => {
    ...
    expect(live.doors.map((d) => `${d.at} -> ${d.value}`).sort()).toEqual([
      ...
      "src/workspace/Workspace.ts:3503 -> APPROVAL_CHANNEL_COMPANION_HTTP",
    ]);
  });
  ...
  it("RED at both write doors: an actor injected into either real call site is refused", () => {
    ...
    expect(door2.findings.map((f) => ({ at: f.at, problem: f.problem }))).toEqual([
      { at: "src/workspace/Workspace.ts:3503", problem: LITERAL_ACTOR },
      { at: "src/workspace/Workspace.ts:3503", problem: HARD_CODED },
    ]);
